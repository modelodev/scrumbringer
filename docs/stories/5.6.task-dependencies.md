...
10. **AC10**: Una tarea bloqueada PUEDE ser reclamada (soft block, no hard block)
11. **AC11**: Al reclamar tarea bloqueada, se muestra advertencia: "Esta tarea depende de N tareas incompletas. Continuar?"
12. **AC12**: Cuando todas las dependencias se completan, la tarea queda desbloqueada automaticamente

### API

13. **AC13**: `GET /api/v1/tasks/:id/dependencies` - Lista tareas de las que depende
14. **AC14**: `POST /api/v1/tasks/:id/dependencies` - Body: `{ depends_on_task_id: Int }`
15. **AC15**: `DELETE /api/v1/tasks/:id/dependencies/:dep_task_id` - Elimina dependencia
16. **AC16**: `GET /api/v1/projects/:id/tasks` acepta filtro `?blocked=false` (solo desbloqueadas)

### Validaciones

17. **AC17**: No se puede crear dependencia circular (A depende de B, B depende de A)
18. **AC18**: No se puede crear dependencia a tarea de otro proyecto
19. **AC19**: No se puede crear dependencia a tarea ya completada (no tiene sentido)

### Modelo de Datos

20. **AC20**: Tabla `task_dependencies(task_id, depends_on_task_id, created_at, created_by)`
21. **AC21**: Indices para busqueda rapida en ambas direcciones

### i18n

22. **AC22**: Labels en ES/EN:
...
+----------------------------------------------------------+
| [!] Configurar credenciales    [Pendiente]  [x]           |
| [!] Diseñar flujo de auth      [En curso]   [x]           |
