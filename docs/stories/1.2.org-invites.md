...
## Story
**As an** org admin,
**I want** to create invite tokens for my organization,
...
4. **Response shape**: On success, `POST /api/v1/org/invites` returns `200` `{ data: { invite } }` where `invite` includes:
   ...
   - `url_path` (relative path; no scheme/host)
   - `created_at`
   - `expires_at`
...
7. **Opaque token**: Invite `token` is treated as an opaque, URL-safe random token (no embedded meaning).
...
- [x] Implement persistence for org invites in PostgreSQL (requires Story 1.2a) (AC: 2, 3, 4, 5, 6)
  - [x] Store `token`, `org_id`, `created_by`, `created_at`, `expires_at`, `used_at`, `used_by` (AC: 4, 5, 6)
  - [x] Ensure invite `token` is unique and URL-safe (AC: 4, 7)
...
Token guidance (MVP):
- Treat `token` as an **opaque, URL-safe random token** (no embedded meaning).
