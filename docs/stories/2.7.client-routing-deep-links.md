...
2. **Caddy SPA fallback**: Con Caddy delante de la app, las rutas no-API (`/admin/*`, `/app/*`, `/accept-invite`, `/reset-password`, `/`) devuelven `index.html` y los assets se sirven desde `apps/client/dist/`.
   - Las rutas `/api/*` se proxyean al backend (puerto `SB_PORT` leído desde `apps/server/.env`).
...
- [x] Task 1: Añadir Caddy SPA fallback (AC: 1, 2)
  - [x] Crear `Caddyfile` en repo root con:
    - [x] `reverse_proxy localhost:{$SB_PORT}` para `/api/*`.
...
- Note: This story text references `/api/*`; implementation is aligned to `/api/v1/*` routing in the current codebase. Please update story wording in a follow-up.
