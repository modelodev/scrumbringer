...
┌───────────────────────────────────────────────────────────────────────────────┐
│ ▼ M1 · Hito planificado                                 [Listo]  0% █░░░░░░  │
│   0/1 tarjetas · 0/2 tareas                                                 │
│                                                                              │
│   TARJETAS                                                                    │
│   ┌ P3 - Sprint Planning #1                                   0/0 [Mover] ┐ │
│   └─────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
│   [Ver detalle]   [Activar]   [+ Tarjeta]   [+ Tarea]        ✏️      🗑️     │
└───────────────────────────────────────────────────────────────────────────────┘
```
...
1. Rediseño total del design system.
...
Gate: PASS → docs/qa/gates/7.9-hitos-ui-consistency-and-non-stacked-create.yml

### Recommended Status

[✓ Ready for Done]

### Review Date: 2026-02-09 (Dev/QA Sweep Final)

### Reviewed By: James (Dev) + Playwright evidence sweep

### Sprint-7 Functional/Visual Sweep (a/b/c)

| Flujo | Resultado | Evidencia |
| ----- | --------- | --------- |
| a) Crear tarea directa en proyecto | PASS | `/run/user/1000/agent-browser/tmp/screenshots/screenshot-2026-02-08T22-46-40-055Z-r0egvq.png` |
| a) Crear tarea vinculada a tarjeta (selector de tarjeta en create-task) | PASS | `/run/user/1000/agent-browser/tmp/screenshots/screenshot-2026-02-08T22-46-59-203Z-234434.png` |
| b) Crear tarjeta directa en proyecto | PASS | `/run/user/1000/agent-browser/tmp/screenshots/screenshot-2026-02-08T22-49-34-730Z-jatui5.png` |
| b) Crear tarjeta contextual en hito (`+ Tarjeta`) | PASS | `/run/user/1000/agent-browser/tmp/screenshots/screenshot-2026-02-08T22-52-49-988Z-t4zkb9.png` |
| c) Crear hitos A/B + mover tarjeta entre hitos | PASS | `/tmp/s7-sweep-evidence/06-before-a-to-b.png`, `/tmp/s7-sweep-evidence/07-after-a-to-b.png`, `/tmp/s7-sweep-evidence/final-move-report.json` |

### Notas de cierre del sweep

- Se ejecuto barrido final de flujo de movimiento entre hitos con automatizacion Playwright en `https://localhost:8443`.
- Resultado del flujo A -> B: `PASS` (ver `cardVisibleInB: PASS` en `/tmp/s7-sweep-evidence/final-move-report.json`).
- Verificacion de estado actual adicional: `/tmp/s7-sweep-evidence/check-summary.json` y `/tmp/s7-sweep-evidence/05-current-b-detail-check.png`.
- Confianza de cobertura para objetivos sprint-7 (flujos a/b/c): alta, sin bloqueantes funcionales o visuales abiertos en esta historia.
