...
> **Nota**: Funcionalidad core (AC1-AC15) implementada y testeada. Pendiente: mejoras UX (AC16-AC22) con hovers y rediseño del modal.
...
1. **AC1**: La vista detalle de una card tiene sección "Notas" (se accede desde Kanban o listado de Cards; Pool no aplica por definición de arquitectura).
2. **AC1.1**: El detalle de la card se puede abrir tanto desde Kanban como desde `https://localhost:8443/config/cards`.
2. **AC2**: Un miembro puede añadir una nota con texto libre (sin selección manual de tipo).
3. **AC3**: Las notas muestran: autor, fecha y contenido.
4. **AC4**: Las notas se ordenan cronológicamente (más reciente al final).
...
8. **AC8**: El indicador visual desaparece inmediatamente después de que el usuario abre el detalle de la Card (limpieza por lectura personal).
...
12. **AC12**: `PUT /api/v1/views/cards/:id` - Marca la card como "leída" para el usuario actual.
...
16. **AC16**: Hover en indicador `[!]` muestra preview de notas nuevas (cantidad, tiempo, última nota truncada y autor).
17. **AC17**: Hover en título de card truncado muestra el título completo.
18. **AC18**: Hover en barra de progreso muestra desglose de tareas (completadas, en curso, pendientes).
19. **AC19**: Hover en botón eliminar nota muestra "Eliminar nota" (o "Eliminar nota (como admin)" si no es el autor).
...
22. **AC22**: Hover en iconos de acción (✏️ 🗑️) en tabla admin muestra label descriptivo ("Editar", "Eliminar card").

#### Matriz de Hovers por Vista

| AC | Hover | `/config/cards` | `/fichas` | `/lista` | Card Detail Modal | Objetivo |
|----|-------|:---------------:|:---------:|:--------:|:-----------------:|----------|
| AC16 | Indicador `[!]` | ✓ | ✓ | ✓ | - | Previsualizar notas sin abrir el modal |
| AC17 | Título truncado | ✓ | ✓ | ✓ | - | Ver título completo sin abrir detalle |
| AC18 | Barra progreso | ✓ | - | - | ✓ (header) | Entender estado de tareas sin contar |
| AC19 | Botón eliminar nota | - | - | - | ✓ (tab notas) | Confirmar acción y clarificar permisos |
| AC20 | Nombre autor | - | - | - | ✓ (tab notas) | Identificar al autor para contacto |
| AC21 | Tab con contador | - | - | - | ✓ | Saber si hay notas nuevas para mí |
| AC22 | Iconos acción tabla | ✓ | - | - | - | Accesibilidad: describir acción del icono |
...
```
┌──────────────────────────────────────────┐
│  🟢 Sprint Planning Q1        Pendiente  │
│  [!]                                     │
│   ┌─────────────────────────────────┐    │
│   │ 💬 2 notas nuevas               │    │
│   │    desde hace 3 horas           │    │
│   │                                 │    │
│   │ Última: "Revisé los mockups..." │    │
│   │         — María G.              │    │
│   └─────────────────────────────────┘    │
└──────────────────────────────────────────┘
```

**AC17 - Hover en título truncado:**
```
┌──────────────────────────────────────────┐
│  🟡 Documentation Up...       Pendiente  │
│   ┌─────────────────────────────────┐    │
│   │ Documentation Update for v2.0  │    │
...
**AC19 - Hover en botón eliminar nota:**
```
┌─────────────────────────────────────────────────┐
│  👤 Carlos López                      09:15    │
│  │ Necesitamos definir edge cases.          │  │
│                                         [🗑️]   │
│                       ┌─────────────────────┐  │
│                       │ Eliminar nota       │  │
│                       │ (como admin)        │  │
...
**AC22 - Hover en iconos de acción:**
```
┌──────────────┐         ┌──────────────┐
│  ✏️  🗑️      │         │  ✏️  🗑️      │
│  ┌────────┐  │         │     ┌─────────────┐
...
> **Nota**: La pestaña "Actividad" fue eliminada (no se usa actualmente). El modal solo tiene dos pestañas: TAREAS y NOTAS.
...
│  │  ─────────────────────────────────────────────────────── │   │
│  │  ┌──────────┐  ┌──────────────────────────────────────┐ │   │
│  │  │Pendiente │  │ 3/5 tareas completadas    ████████░░ │ │   │
│  │  └──────────┘  └──────────────────────────────────────┘ │   │
│  │                                                          │   │
│  │  ┌─────────────────────────┬─────────────────────────┐  │   │
│  │  │        TAREAS           │        NOTAS (3)        │  │   │
...
│  │  │                                                      │   │
│  │  │  ┌────────────────────────────────────────────────┐ │   │
│  │  │  │ + Añadir tarea a esta card...                  │ │   │
│  │  │  └────────────────────────────────────────────────┘ │   │
│  │  │                                                      │   │
│  │  │  ┌────────────────────────────────────────────────┐ │   │
│  │  │  │ ○  Configurar entorno de staging               │ │   │
│  │  │  │    🏷️ DevOps  ⚡ P2  👤 Sin asignar            │ │   │
│  │  │  └────────────────────────────────────────────────┘ │   │
│  │  │                                                      │   │
│  │  │  ┌────────────────────────────────────────────────┐ │   │
│  │  │  │ ◐  Implementar login OAuth                     │ │   │
│  │  │  │    🏷️ Backend  ⚡ P1  👤 Carlos                │ │   │
│  │  │  │    ⏱️ En curso desde hace 2h                   │ │   │
│  │  │  └────────────────────────────────────────────────┘ │   │
│  │  │                                                      │   │
│  │  │  ┌────────────────────────────────────────────────┐ │   │
│  │  │  │ ●  Crear wireframes                            │ │   │
│  │  │  │    🏷️ UX  ⚡ P1  👤 María  ✓ Completada ayer  │ │   │
│  │  │  └────────────────────────────────────────────────┘ │   │
│  │  │                                                      │   │
...
│  │  │  │  👤 María García                      14:32    │ │   │
│  │  │  │  ┌────────────────────────────────────────────┐│ │   │
│  │  │  │  │ Revisé los mockups con el cliente.        ││ │   │
│  │  │  │  │ Aprobaron la versión 2.                   ││ │   │
│  │  │  │  └────────────────────────────────────────────┘│ │   │
│  │  │  │                                         [🗑️]   │ │   │
│  │  │  └────────────────────────────────────────────────┘ │   │
│  │  │                                                      │   │
...
│  │  │  │  👤 Carlos López                      09:15    │ │   │
│  │  │  │  ┌────────────────────────────────────────────┐│ │   │
│  │  │  │  │ Necesitamos definir los edge cases.       ││ │   │
│  │  │  │  └────────────────────────────────────────────┘│ │   │
│  │  │  │                                         [🗑️]   │ │   │
│  │  │  └────────────────────────────────────────────────┘ │   │
│  │  │                                                      │   │
...
- Altura mínima: 4 líneas
- Contador de caracteres: `0/500` alineado abajo-derecha
- Botón "Añadir nota" alineado a la derecha bajo el textarea
- Placeholder: "Nueva nota..." / "New note..."

#### Empty State para Notas (AC2)

Cuando no hay notas, mostrar un estado vacío amigable:
...
│              Sin notas todavía                           │
│                                                          │
│        Las notas ayudan a tu equipo a mantener          │
│          contexto sobre esta card.                       │
│                                                          │
│  ┌────────────────────────────────────────────────────┐ │
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  📋 TARJETAS - PROJECT ALPHA                        [+ Crear tarjeta]  │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  🔍 Buscar...          Estado: [Todas ▼]  ☐ Vacías  ☐ Completadas      │
│                                                                         │
├──────────────────────────┬──────────┬───────────────────┬──────────────┤
│  TÍTULO                  │  ESTADO  │  PROGRESO         │  ACCIONES    │
├──────────────────────────┼──────────┼───────────────────┼──────────────┤
│  🟢 Sprint Planning Q1   │Pendiente │  ████████░░ 3/5   │  ✏️  🗑️      │
│  [!] 2 notas nuevas      │          │  60%              │              │
├──────────────────────────┼──────────┼───────────────────┼──────────────┤
│  🔵 API Refactor         │En curso  │  ██████████ 5/5   │  ✏️  🗑️      │
│     Creada hace 1 semana │          │  ✓ Completado     │              │
├──────────────────────────┼──────────┼───────────────────┼──────────────┤
│  🟡 Documentation Up...  │Pendiente │  ██░░░░░░░░ 1/8   │  ✏️  🗑️      │
│     Creada hace 3 días   │          │  12%              │              │
└──────────────────────────┴──────────┴───────────────────┴──────────────┘
```

**Especificaciones del indicador [!] (AC7, AC8):**
- Aparece bajo el título de la card si `has_new_notes = true`
- Color: naranja/warning para destacar
- Desaparece al abrir el detalle de la card (AC8)
- Tooltip: "X notas desde tu última visita"
...
│  👤 Nombre del autor                      HH:MM         │
│  ┌────────────────────────────────────────────────────┐ │
│  │ Contenido de la nota con soporte para             │ │
│  │ múltiples líneas de texto.                        │ │
│  └────────────────────────────────────────────────────┘ │
│                                                  [🗑️]   │  ← Solo visible si:
│                                                         │     - Usuario es autor (AC6)
└─────────────────────────────────────────────────────────┘     - Usuario es admin (AC5)
```

**Especificaciones de agrupación (AC4):**
- Notas agrupadas por día: "Hoy", "Ayer", o fecha específica
- Orden cronológico: más antiguas arriba, más recientes abajo
- Separador visual entre grupos de días

## Tasks / Subtasks

- [x] Database (AC: 13, 14)
  - [x] Crear migración `dbmate` para `card_notes`.
  - [x] Crear migración para `user_card_views`.
...
- [x] Frontend UI (AC: 1, 2, 3, 4, 7, 8)
  ...
  - [x] Al abrir el detalle, disparar el comando para marcar como leída (`AC11`).
...
- [ ] UX - Card Detail Modal Layout (deferred to 5.4)
  ...
  - [ ] Redimensionar textarea de notas a 4 líneas mínimo.
  - [ ] Añadir contador de caracteres (0/500) en textarea.
  - [ ] Alinear botón "Añadir nota" a la derecha bajo textarea.
  - [ ] Implementar empty state amigable para notas vacías.
...
- [x] UX - Hovers en Listados (AC: 16, 17, 22)
  ...
  - [x] Hover en título truncado: mostrar título completo en tooltip (via text_utils.truncate).
  - [x] Hover en iconos de acción (✏️ 🗑️): mostrar label descriptivo (via action_buttons).
...
- [x] UX - Hovers en Card Detail Modal (AC: 18, 19, 20, 21)
  ...
  - [x] Hover en botón eliminar nota: "Eliminar" o "Eliminar (como admin)" según contexto.
  - [x] Hover en nombre de autor: (estructura del componente `author_tooltip` creada).
  - [x] Hover en tab con contador: (componente `tab_badge` creado, pendiente integración en tabs).
- [x] UX - Limpieza de código y consistencia visual
  - [x] Removida pestaña "Activity" (no usada) con todo el código asociado.
  - [x] Creado componente `card_section_header.gleam` compartido para ambas pestañas.
  ...
  - [x] **Diálogo de crear nota**: el botón "+ Add note" abre un diálogo modal en lugar de inline.
  - [x] Patrón DRY: ambas pestañas usan el mismo patrón (header con botón → abre diálogo).
  - [x] Removidos i18n keys: TabActivity, ComingSoon.
- [x] i18n - Labels de Hovers (AC: 15 extensión)
  - [x] Añadir traducciones ES/EN para tooltips: NewNotesTooltip, EditCardTooltip, DeleteCardTooltip, ProgressTooltip.
...
### Technical Context
- **Tracking de Lectura**: Usar la "Opción B" (tabla de relación) para que el indicador sea personal por usuario.
...
### Testing Standards
- Seguir los estándares de `docs/architecture/testing-architecture.md`.
...
/// AC19 - Contexto de borrado para tooltip del botón eliminar
...
| AC | Componente | Ubicación | Reutiliza |
|----|------------|-----------|-----------|
| AC16 | `notes_preview_tooltip` | `ui/tooltips/notes_preview_tooltip.gleam` | Nuevo |
| AC17 | `truncated_text` | `components/truncated_text.gleam` | **Existente** ✓ |
...
| AC22 | Atributo `title` | N/A | **Patrón existente** ✓ |

### Arquitectura de Tooltips

**Decisión**: Usar componentes específicos por tipo de contenido (no un tooltip genérico).
...
- Alineado con patrón existente (`task_hover_popup.gleam`)
- Facilita testing unitario por componente

**Patrón a seguir** (basado en `task_hover_popup.gleam`):
```gleam
pub type Config(msg) {
  Config(
    data: DataType,
...
```
apps/client/test/
├── ui_notes_preview_tooltip_test.gleam   # AC16
├── ui_progress_tooltip_test.gleam        # AC18
├── ui_author_tooltip_test.gleam          # AC20
├── ui_tab_badge_test.gleam               # AC21
├── notes_list_delete_context_test.gleam  # AC19
└── card_detail_modal_tabs_test.gleam     # Modal tabs
```
...
// Given: NotesPreviewData con last_note_preview=Some("Revisé...")
// When: view(config)
// Then: HTML contiene "Última:" y "Revisé..."

pub fn hides_last_note_when_none_test()
// Given: NotesPreviewData con last_note_preview=None
// When: view(config)
// Then: HTML no contiene "Última:"
```
...
// Given: Vista de tabla con iconos de acción
// When: Render view
...
| 2026-01-28 | 2.1 | Status → In Progress pending UX implementation | Sarah (PO) |
...
- Coding Standards: ✓
- Project Structure: ✓
- Testing Strategy: ✗ (missing view-tracking/indicator coverage; lint not run)
- All ACs Met: ✗ (AC7, AC11, AC13)
...
### Gate Status
Gate: FAIL → docs/qa/gates/5.3-notas-en-cards-fichas.yml
...
### Code Quality Assessment
Added automated coverage for note ordering and i18n labels, including a server-side ordering check. QA gaps are closed with current evidence.
...
### Improvements Checklist
- [x] Add test coverage for note ordering (AC4).
- [x] Add i18n label coverage (AC15).
- [x] Add server ordering verification (AC4).
...
### Gate Status
Gate: PASS → docs/qa/gates/5.3-notas-en-cards-fichas.yml
...
- Coding Standards: ✓
- Project Structure: ✓
- Testing Strategy: ✗ (missing lint run; UI/i18n/order behavior not fully validated)
- All ACs Met: ✗ (manual UI verification and AC15 validation missing)
...
### Gate Status
Gate: CONCERNS → docs/qa/gates/5.3-notas-en-cards-fichas.yml
