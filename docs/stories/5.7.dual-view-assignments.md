# Story 5.7: Dual View - Vista Híbrida de Asignaciones Usuarios/Proyectos
...
**so that** puedo gestionar la membresía de usuarios en múltiples proyectos de forma eficiente sin navegar entre vistas separadas.

## Problem Statement

Actualmente la gestión de usuarios multi-proyecto está fragmentada en tres vistas desconectadas:

1. **`/org/projects`** - Lista proyectos con CRUD básico, sin visibilidad de miembros
2. **`/org/users`** - Lista usuarios con diálogo modal para ver/añadir proyectos (lazy loading, UX pobre)
3. **`/admin/members?project=X`** - Miembros por proyecto (requiere seleccionar proyecto primero en sidebar)

**Problemas identificados:**

- No hay forma de ver "¿en qué proyectos está María?" sin abrir un modal
- No hay forma de ver "¿quién tiene acceso a Project Beta?" sin cambiar de vista
- Onboarding de nuevos empleados requiere múltiples navegaciones y modales
- Usuarios sin proyectos y proyectos vacíos no son visibles fácilmente
- La información de proyectos en la tabla de usuarios muestra "..." hasta abrir el modal

**Nota de integración:** Esta nueva vista **no reemplaza** `/org/users` ni `/org/projects`. Ambas rutas se mantienen sin cambios.

## Objetivos

1. **Vista unificada**: Una sola ruta `/org/assignments` que consolide la gestión de asignaciones
2. **Perspectiva dual**: Toggle para cambiar entre "Por Proyecto" y "Por Usuario"
3. **Edición inline**: Añadir/quitar usuarios de proyectos sin modales anidados
...
```
┌─────────────────────────────────────┐
│ ▣ Por Proyecto │ ☐ Por Usuario      │    🔍 Buscar...
└─────────────────────────────────────┘
```
...
┌─ 📁 Project Alpha ──────────────────────────────────────────────────────────┐
│                                                                              │
│   👤 admin@example.com        ⚙️ Manager     [Cambiar rol ▾]  [✕ Quitar]    │
│   👤 pm@example.com           👤 Miembro     [Cambiar rol ▾]  [✕ Quitar]    │
│   👤 member@example.com       👤 Miembro     [Cambiar rol ▾]  [✕ Quitar]    │
│                                                                              │
│   ┌──────────────────────────────────────────────────────────────────────┐  │
│   │  + Añadir usuario   🔍 [Buscar email...              ]  [Manager ▾]  │  │
│   └──────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│                                                    [✎ Editar] [🗑 Eliminar] │
└──────────────────────────────────────────────────────────────────────────────┘
...
```
┌─ 👤 admin@example.com ─────────────────────────────────────── Rol Org: Admin ┐
│                                                                              │
│   📁 Project Alpha            ⚙️ Manager     [Cambiar rol ▾]  [✕ Quitar]    │
│   📁 Project Beta             ⚙️ Manager     [Cambiar rol ▾]  [✕ Quitar]    │
│                                                                              │
│   ┌──────────────────────────────────────────────────────────────────────┐  │
│   │  + Añadir a proyecto   [Seleccionar proyecto...  ▾]      [Manager ▾] │  │
│   └──────────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────────────┘

┌─ 👤 nuevo@example.com ─── ⚠️ SIN PROYECTOS ───────────────── Rol Org: Member ┐
│                                                                              │
...
- **Usuario sin proyectos**: Badge `⚠️ SIN PROYECTOS` en header de card
- **Proyecto sin miembros**: Badge `⚠️ SIN MIEMBROS` + mensaje explicativo

## Acceptance Criteria

### Core

1. Nueva ruta `/org/assignments` accesible desde sidebar bajo "ORGANIZACIÓN"
2. Toggle funcional entre "Por Proyecto" y "Por Usuario" que preserva filtro de búsqueda
...
5. Búsqueda unificada filtra por nombre de proyecto O email de usuario en ambas vistas

### Gestión Inline

6. Formulario inline "Añadir usuario" en cada card de proyecto (sin modal)
7. Formulario inline "Añadir a proyecto" en cada card de usuario (sin modal)
8. Dropdown para cambiar rol de usuario en proyecto (member/manager) con guardado automático
9. Botón para quitar usuario de proyecto con confirmación inline (no modal)

### Indicadores y UX

10. Badge visual `⚠️ SIN PROYECTOS` en cards de usuarios sin asignaciones
11. Badge visual `⚠️ SIN MIEMBROS` en cards de proyectos vacíos
...
15. Áreas tappable mínimo 44x44px
...
- [x] **Task 1: Estructura base y routing** (AC: 1, 15)
  - [x] 1.1 Añadir nueva ruta `/org/assignments` en router
  - [x] 1.2 Crear módulo `features/assignments/view.gleam`
  - [x] 1.3 Crear módulo `features/assignments/update.gleam`
  - [x] 1.4 Añadir `AssignmentsModel` al estado de admin
  - [x] 1.5 Verificar permisos de org admin en guard de ruta
  - [x] 1.6 Añadir entrada en sidebar bajo "ORGANIZACIÓN"
...
- [x] **Task 2: Toggle y estado de vista** (AC: 2)
  ...
  - [x] 2.4 Preservar búsqueda al cambiar de modo
...
- [x] **Task 4: Vista Por Proyecto** (AC: 3, 6, 8, 9, 11)
  ...
  - [x] 4.2 Reutilizar formulario inline "Añadir usuario"
  - [x] 4.3 Reutilizar dropdown de cambio de rol
  - [x] 4.4 Reutilizar botón quitar con confirmación inline
  - [x] 4.5 Añadir badge `⚠️ SIN MIEMBROS` para proyectos vacíos
  - [x] 4.6 Añadir acciones de proyecto (editar nombre, eliminar)
...
- [x] **Task 5: Vista Por Usuario** (AC: 4, 7, 8, 9, 10)
  ...
  - [x] 5.2 Reutilizar formulario inline "Añadir a proyecto"
  - [x] 5.3 Reutilizar dropdown de cambio de rol
  - [x] 5.4 Reutilizar botón quitar
  - [x] 5.5 Añadir badge `⚠️ SIN PROYECTOS` para usuarios sin asignaciones
...
- [x] **Task 6: Búsqueda** (AC: 5)
  - [x] 6.1 Implementar input de búsqueda con debounce
...
- [x] **Task 7: API y datos** (AC: 3, 4, 6, 7, 8, 9)
  - [x] 7.1 Reutilizar endpoints existentes para CRUD de membresías (base `/api/v1`)
...
- [x] **Task 8: Responsive y accesibilidad** (AC: 14, 15)
  ...
  - [x] 8.2 Áreas tappable 44x44px mínimo
  - [x] 8.3 Focus management para navegación por teclado
...
- Usar `icons.nav_icon()` para iconos y tamaños predefinidos. [Source: docs/architecture/coding-standards.md#L278]
...
1. Toggle cambia vista y preserva búsqueda
...
### Enumeración de tests (lenguaje natural)
...
3. Cambiar el toggle de Por Proyecto a Por Usuario conserva el texto de búsqueda.
...
6. Si no hay proyectos, la vista Por Proyecto muestra el empty state con CTA “Crear proyecto”.
7. Si no hay usuarios, la vista Por Usuario muestra el empty state con CTA “Invitar usuario”.
8. Un proyecto sin miembros muestra el badge “⚠️ SIN MIEMBROS”.
9. Un usuario sin proyectos muestra el badge “⚠️ SIN PROYECTOS”.
...
13. Quitar usuario de proyecto requiere confirmación inline antes de enviar la solicitud.
...
4. **Warning badge**: Usuario sin proyectos muestra `⚠️ SIN PROYECTOS`
5. **Warning badge**: Proyecto sin miembros muestra `⚠️ SIN MIEMBROS`
...
- Vista matricial (usuarios × proyectos)
- Edición de rol de organización (se mantiene en /org/users)
- Gestión de capacidades de usuario (se mantiene en /admin/members)
...
1. **Riesgo de duplicación/confusión de flujos**
   - Mitigación: mantener `/org/users` y `/org/projects` sin cambios y documentar que `/org/assignments` es adicional.

2. **Riesgo de permisos inconsistentes**
   - Mitigación: reutilizar el guard admin-only de `/org/users` y tests de permisos.

3. **Riesgo de rendimiento por múltiples requests**
   - Mitigación: fetch paralelo y loading local por card para evitar bloqueo global.
...
|-------|---------|-------------|-------|
| 2026-01-27 | 1.0 | Historia inicial creada desde análisis UX | Sarah (PO) |
...
- Coding Standards: ✓
- Project Structure: ✓
- Testing Strategy: ✓ (unit tests added; E2E via Playwright headless)
- All ACs Met: ✓ (AC18 accepted as not observable under fast local fetch)
...
Gate: PASS → docs/qa/gates/5.7-dual-view-assignments.yml
...
- Coding Standards: ✗ Not independently verified; lint not run (pre-existing detached doc comment warnings noted in `apps/client/src/scrumbringer_client/client_update.gleam` and admin views).
- Project Structure: ✓ No deviations noted in Dev Agent Record; not independently verified.
- Testing Strategy: ✗ Manual UI verification not performed; only automated client tests reported.
- All ACs Met: ✓ Based on reported tests and completion notes; no manual UI verification.
...
Gate: CONCERNS → docs/qa/gates/5.7-dual-view-assignments.yml
