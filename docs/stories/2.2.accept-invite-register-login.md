...
- [ ] Extend registration to accept `invite_token` (AC: 3, 4, 5)
   - [ ] Enforce post-bootstrap invite-only registration using `invite_token` only (no legacy interfaces) (AC: 3)
...
- [ ] Implement Accept-invite UI (AC: 6, 7)
  ...
  - [ ] Implement token validation request + decoding (AC: 1, 6)
   - [ ] Implement password form + submit to `POST /api/v1/auth/register` with `{ password, invite_token }` (AC: 3, 6)
