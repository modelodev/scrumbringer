...
### Reuse / DRY (alineado a 5.3)

- Reusar `ui/notes_list` y `ui/notes_composer` definidos en 5.3.
- Mantener `link_preview` y `url_detector` como helpers compartidos solo si tambien se usan en notas de cards.
- Evitar una capa "NotesService" global si no aporta reutilizacion real.

### TDD (Red/Green/Refactor)

- Red: tests para `note_type` en POST/GET y rendering de estilos (delivery/blocked).
- Green: implementar templates + deteccion de URLs hasta pasar tests.
- Refactor: extraer helpers comunes con 5.3 cuando haya duplicacion real.

### Checklist TDD

Backend
- [ ] POST acepta `note_type` opcional y lo persiste.
- [ ] GET retorna `note_type` cuando existe.
- [ ] Migracion `note_type` permite NULL sin romper notas existentes.

Frontend
- [ ] Templates aplican texto base editable.
- [ ] Estilos: delivery (verde), blocked (amarillo).
- [ ] URLs detectadas y renderizadas (GitHub con icono, generico con dominio).
