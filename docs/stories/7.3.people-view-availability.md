...
1. Se agrega una vista **Personas** en la seccion TRABAJO con orden de navegación: **Pool - Kanban - Personas - Hitos**.
...
7. La navegacion de Personas reutiliza el patron existente de `ViewMode` en sidebar y center panel, sin rutas o toggles paralelos (NO crear `/app/people`; usar `router.Member(member_section.Pool, state)` con `view=people`). Etiquetas UI requeridas: `Pool`, `Kanban` (antes `Tarjetas`), `Personas`, `Hitos` en ese orden.
...
| 2026-02-06 | 1.6 | Ajuste de nomenclatura/orden en sidebar: `Tarjetas` pasa a `Kanban`, orden final `Pool - Kanban - Personas - Hitos` | Sarah (PO) |
...
| 2026-02-06 | 2.0 | Normalizacion PO de estado BMAD: Ready for Review -> Done tras cierre formal de 7.3 | Sarah (PO) |
