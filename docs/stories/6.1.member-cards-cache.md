...
- [x] Task 1 (AC: 1, 2, 5)
  - [x] Agregar `member_cards_store: NormalizedStore(Int, Card)` al `MemberModel`.
  - [x] Inicializar nuevos campos en `default_model()`.

- [x] Task 2 (AC: 1, 2, 4)
  - [x] Implementar `NormalizedStore` opaco en `state/normalized_store.gleam`.
  - [x] Asegurar firmas publicas con tipos explicitos.

- [x] Task 3 (AC: 5, 6)
  - [x] Emitir `MemberProjectCardsFetched(project_id, result)` por cada proyecto en `refresh_member_data`.
  - [x] Eliminar el branch de cards solo para selected_project_id.

- [x] Task 4 (AC: 1, 2, 6)
  - [x] Manejar `MemberProjectCardsFetched` usando `store.upsert` + `store.is_ready` + `store.to_list`.
  - [x] Mantener cache en errores parciales.

- [x] Task 5 (AC: 3, 4)
  - [x] Refactorizar `card_queries.find_card` para usar `member_cards_store`.
  - [x] Refactorizar `card_queries.get_project_cards` para usar `store.get_by_project`.

- [x] Task 6 (AC: 3, 7)
  - [x] Actualizar `features/fichas/view.gleam` para usar las nuevas queries.
  - [x] Verificar que la vista usa cache si existe.

- [x] Task 7 (AC: 1-7)
  - [x] Implementar tests obligatorios (seccion inferior).
...
| 2026-02-02 | 1.1 | Implementacion de cache normalizado y tests de soporte | Dev |
...
- 2026-02-02: `cd apps/client && gleam test`

### Completion Notes List

- Normalized store opaco con indices por proyecto y pending para cards.
- Refresh multi-proyecto para cards con handlers que preservan cache ante errores.
- Queries y vista de fichas ahora leen del store y respetan cache/pending.
- Suite de tests agregada para store, update, queries y view.
- Verificacion manual de UI no realizada.
...
- apps/client/test/member_cards_skills_view_test.gleam
...
Normalized store, multi-proyecto refresh y handlers de cache implementados con enfoque TEA limpio; queries/vista usan el store y los tests cubren los escenarios clave.
...
- Coding Standards: ✓ (patrones TEA, helpers puros)
- Project Structure: ✓ (ubicaciones y nombres correctos)
- Testing Strategy: ✓ (tests unit y view añadidos)
- All ACs Met: ✓ (segun tests y lectura de implementacion)

### Improvements Checklist

- [x] Normalized store con indices y pending
- [x] Refresh multi-proyecto y handlers de cache
- [x] Queries/vista usan store
- [x] Tests unit/update/query/view agregados
- [ ] Verificacion manual UI en Fichas/Sidebar

### Security Review

Sin cambios de seguridad; no se introducen riesgos nuevos.

### Performance Considerations

Mejora O(1) en lookup por id y evita re-renders innecesarios por recomputos globales; sin regresiones observadas.
...
Gate: CONCERNS → docs/qa/gates/6.1-cache-de-fichas-normalizado-arquitectura-de-alto-rendimiento.yml
...
Playwright headless valida render de tarjetas en Project 7/8 y cambio de proyecto mantuvo titulos. Selector correcto es `data-testid="card-title"` (kanban view).
...
- [x] Verificacion UI con seed en view=cards
...
Sin regresiones observadas durante navegacion automatizada.
...
Gate: PASS → docs/qa/gates/6.1-cache-de-fichas-normalizado-arquitectura-de-alto-rendimiento.yml
...
Playwright headless confirma acceso a /app/fichas y cambio de proyecto sin errores, pero no hay tarjetas cargadas para verificar comportamiento de cache en UI.
...
- Coding Standards: ✓
- Project Structure: ✓
- Testing Strategy: ✓ (tests unitarios)
- All ACs Met: ⚠ (cache UI no verificable sin datos)

### Improvements Checklist

- [x] Ejecucion de Playwright headless en /app/fichas
- [ ] Validar cache UI con tarjetas reales
...
Sin regresiones observadas durante la navegacion automatizada.
