...
├─────────────────────────────────────────────────────────────────────────────┤
│  Gestiona los usuarios y sus roles en la organización.                       │
│                                                                              │
│  ┌────────────────────┬──────────────┬────────────────────────┬───────────┐ │
│  │ EMAIL              │ ROL ORG      │ PROYECTOS              │ ACCIONES  │ │
│  ├────────────────────┼──────────────┼────────────────────────┼───────────┤ │
│  │ admin@example.com  │ 🟣 Admin     │ 2: Alpha, Beta         │ [Gestionar│ │
│  ├────────────────────┼──────────────┼────────────────────────┼───────────┤ │
│  │ pm@example.com     │ [Member ▼] * │ 2: Alpha (mgr), Beta   │ [Gestionar│ │
│  ├────────────────────┼──────────────┼────────────────────────┼───────────┤ │
│  │ member@example.com │ [Member ▼]   │ 1: Alpha               │ [Gestionar│ │
│  ├────────────────────┼──────────────┼────────────────────────┼───────────┤ │
│  │ new@example.com    │ [Member ▼]   │ Sin proyectos          │ [Gestionar│ │
│  └────────────────────┴──────────────┴────────────────────────┴───────────┘ │
│                                                                              │
│  * cambio pendiente                              [Guardar cambios de rol]    │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| ROL ORG | Dropdown (admin/member) | Shows badge for Admin (🟣). Asterisk (*) if pending change |
| PROYECTOS | Summary text | **Lazy loaded**: Shows "..." until dialog is opened, then caches. Format: "{count}: {names}" or "Sin proyectos". Shows "(mgr)" suffix for manager role |
| ACCIONES | Single button | "Gestionar" opens user projects dialog and loads projects |
...
│  AÑADIR A PROYECTO                                                           │
│                                                                              │
│  Proyecto: [Seleccionar proyecto ▼]   Rol: [Member ▼]   [+ Añadir]          │
│                                                                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                              [Cerrar]        │
└─────────────────────────────────────────────────────────────────────────────┘
...
| Add form - Añadir | Adds user to selected project with selected role |
...
|---------|-----------|-----------------|--------|
| View table | ✅ | ❌ Hidden section | ❌ Hidden section |
| Change org role | ✅ | N/A | N/A |
| Open user projects dialog | ✅ | N/A | N/A |
...
|----------|--------|---------|--------|
| `/api/v1/org/users` | GET | List org users | ✅ Exists |
| `/api/v1/org/users/:id` | PATCH | Update org role | ✅ Exists |
| `/api/v1/org/users/:id/projects` | GET | List user's projects | ✅ Exists |
| `/api/v1/org/users/:id/projects` | POST | Add user to project | ✅ Exists (needs role param) |
| `/api/v1/org/users/:id/projects/:pid` | DELETE | Remove from project | ✅ Exists |
...
| `AddToProjectWithRole` | "Añadir a proyecto" | "Add to project" |
...
│                                                                              │
│            Usa el formulario de abajo para añadirlo a un proyecto            │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│  AÑADIR A PROYECTO                                            │
│                                                               │
│  El usuario ya pertenece a todos los proyectos disponibles.   │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
...
| 2026-01-21 | 1.1 | Clarified lazy loading for PROYECTOS column | Sarah (PO) |
