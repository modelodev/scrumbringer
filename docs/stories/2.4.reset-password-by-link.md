...
1. **Request reset token (public)**: `POST /api/v1/auth/password-resets` generates a reset token for an email.
   - Body: `{ email }`.
   - Response: `200` `{ data: { reset: { email, token, url, created_at } } }`.
...
6. **Unknown email does not leak**: `POST /api/v1/auth/password-resets` does not reveal whether an email exists (still returns `200`), but the returned token is only valid if the account exists.
7. **Error codes are consistent**: invalid/used/invalidated tokens are rejected with a stable error code (suggested):
   - `RESET_TOKEN_INVALID` (403)
   - `RESET_TOKEN_USED` (403)
8. **Client UX**:
   - Login screen provides a “Forgot password?” flow to request a reset link.
   - Client provides a reset-password screen reachable by URL that validates token and submits new password.
9. **No SMTP/email integration**: the reset link is shown directly in the UI for copy/paste; server sends no emails.
...
- [ ] Implement client flow (AC: 8, 9)
  - [ ] Add “Forgot password?” dialog/screen on Login (request by email; show returned URL with copy) (AC: 8, 9)
