...
- [x] QA fixes (post-review)
  - [x] Add rate limiting to password reset endpoints (SEC-001)
  - [x] Add server test for short password on consume (TEST-001)
...
| 2026-01-14 | 1.3 | Apply QA fixes (rate limiting + password policy test) | dev |
...
- QA follow-up: added rate limiting to password reset endpoints and added an explicit server test for short passwords on consume.
...
- QA fixes aplicados: rate limiting añadido para request/validate/consume y test server-side explícito para password < 12.
- Con esto se mitigan los riesgos de abuso y se cierra el gap de coverage de política de password.
...
- [x] Añadir rate limiting a endpoints públicos de password reset (request + validate + consume)
- [x] Añadir test server-side explícito para password < 12 en `/auth/password-resets/consume`

### Security Review

- PASS: rate limiting presente en endpoints públicos; mantiene no-leak para emails desconocidos.

### Performance Considerations

- PASS: lógica ligera e indexada; rate limiting in-memory best-effort.
...
Gate: PASS → `docs/qa/gates/2.4-reset-password-por-link-sin-email-real.yml`
Risk profile: `docs/qa/assessments/2.4-risk-20260114.md`
NFR assessment: `docs/qa/assessments/2.4-nfr-20260114.md`
Trace matrix: `docs/qa/assessments/2.4-trace-20260114.md`
