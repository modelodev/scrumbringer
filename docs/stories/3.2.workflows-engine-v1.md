...
- [ ] Task 1: DB schema for workflows, rules, templates (AC: 1, 2, 3, 8)
  - [ ] Create migration `db/migrations/20260118XXXXXX_create_workflows.sql`
    - [ ] Table `workflows`
    - [ ] Table `rules`
    - [ ] Table `task_templates`
    - [ ] Join table `rule_templates` (N:M)
    - [ ] Table `rule_executions` (idempotency + logging)
  - [ ] Run `make squirrel` to generate types

- [ ] Task 2: Server - TaskTemplate CRUD (AC: 3, 11)
  - [ ] Create `apps/server/src/scrumbringer_server/http/task_templates.gleam`
  - [ ] Create `apps/server/src/scrumbringer_server/services/task_templates_db.gleam`
  - [ ] Endpoints:
    - [ ] `GET /api/v1/task-templates` (org-scoped) / `GET /api/v1/projects/:id/task-templates` (project-scoped)
    - [ ] `POST /api/v1/task-templates` / `POST /api/v1/projects/:id/task-templates`
    - [ ] `PATCH /api/v1/task-templates/:id`
    - [ ] `DELETE /api/v1/task-templates/:id`

- [ ] Task 3: Server - Workflow CRUD (AC: 1, 10, 11)
  - [ ] Create `apps/server/src/scrumbringer_server/http/workflows.gleam`
  - [ ] Create `apps/server/src/scrumbringer_server/services/workflows_db.gleam`
  - [ ] Endpoints:
    - [ ] `GET /api/v1/workflows` / `GET /api/v1/projects/:id/workflows`
    - [ ] `POST /api/v1/workflows` / `POST /api/v1/projects/:id/workflows`
    - [ ] `PATCH /api/v1/workflows/:id` (including activate/deactivate)
    - [ ] `DELETE /api/v1/workflows/:id`
  - [ ] Implement activation cascade to rules

- [ ] Task 4: Server - Rule CRUD (AC: 2, 5, 6, 11)
  - [ ] Create `apps/server/src/scrumbringer_server/http/rules.gleam`
  - [ ] Create `apps/server/src/scrumbringer_server/services/rules_db.gleam`
  - [ ] Endpoints:
    - [ ] `GET /api/v1/workflows/:workflow_id/rules`
    - [ ] `POST /api/v1/workflows/:workflow_id/rules`
    - [ ] `PATCH /api/v1/rules/:id`
    - [ ] `DELETE /api/v1/rules/:id`
    - [ ] `POST /api/v1/rules/:id/templates` (attach template)
    - [ ] `DELETE /api/v1/rules/:id/templates/:template_id` (detach template)

- [ ] Task 5: Server - Rules engine core (AC: 4, 6, 7, 8, 9, 13)
  - [ ] Create `apps/server/src/scrumbringer_server/services/rules_engine.gleam`
  - [ ] Implement `evaluate_rules(event: StateChangeEvent) -> List(RuleResult)`
  - [ ] Implement `execute_rule(rule, event) -> Result(List(Task), Error)`
  - [ ] Implement variable substitution in templates
  - [ ] Implement idempotency check via `rule_executions`
  - [ ] Implement logging of evaluation outcomes

- [ ] Task 6: Server - Integration with task/card state changes (AC: 4, 9)
  - [ ] Modify `apps/server/src/scrumbringer_server/http/tasks.gleam`:
    - [ ] After claim/release/complete, call `rules_engine.evaluate_rules`
  - [ ] Modify `apps/server/src/scrumbringer_server/http/cards.gleam`:
    - [ ] After task changes that affect card state, evaluate rules
  - [ ] Ensure `user_triggered` flag is passed (skip if false)

- [ ] Task 7: Client - Admin UI for workflows (AC: 12)
  - [ ] Create `apps/client/src/scrumbringer_client/features/admin/workflows.gleam`
  - [ ] Create `apps/client/src/scrumbringer_client/features/admin/rules.gleam`
  - [ ] Create `apps/client/src/scrumbringer_client/features/admin/task_templates.gleam`
  - [ ] Add tabs/sections in Admin UI
  - [ ] Implement:
    - [ ] Workflow list with active toggle
    - [ ] Rule list per workflow with template associations
    - [ ] TaskTemplate list with CRUD
  - [ ] Add i18n keys
