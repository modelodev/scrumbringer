...
### Decision note: task -> card assignment deferred globally

- Se ha considerado anadir en la vista de Hitos una accion para mover una `task` suelta a una `card` o crear una `card` desde una `task`.
- La decision es **posponerla** hasta un diseno global del producto, porque introducirla solo en Hitos dejaria una capacidad de organizacion importante disponible en una unica vista y generaria inconsistencia funcional con `Pool`, `Kanban`, `Capacidades` y los detalles de `task/card`.
- A nivel tecnico, el frontend actual soporta mover `task` entre hitos (`milestone_id`) y crear `task/card` contextualizadas, pero no expone hoy una mutacion transversal clara para reasignar una `task` existente a una `card` desde cualquier vista.
- Por tanto, cualquier implementacion correcta de `task -> card` debe abordarse como una capacidad horizontal con:
  - contrato backend/API explicito,
  - reglas de dominio compartidas,
  - y rollout coherente en todas las vistas relevantes.
- Mientras tanto, la vista de Hitos puede seguir mostrando `tasks` sueltas como excepcion visible y ofrecer solo acciones que ya son consistentes globalmente (crear, mover entre hitos `ready`, activar, editar, borrar).

#### Follow-up recomendado

1. Definir la capacidad global `asignar/mover task a card` a nivel de producto.
2. Disenar su semantica exacta (`task` existente -> `card` existente, `task` -> nueva `card`, herencia de `milestone`, restricciones si la `task` ya esta activa/claimed, etc.).
3. Anadir soporte backend/API dedicado.
4. Desplegarla de forma coherente en `Pool`, `Kanban`, `Capacidades`, `Hitos` y detalles de `task/card`.
