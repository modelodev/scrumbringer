...
```
▌ [🔧]  Fix login button   Reclamada por admin ▶   ⋯
```
...
5. Lista muestra "Reclamada por X" con icono operativo homogéneo (play/pause) al lado del nombre.
...
9. En mobile/tablet, el hover se reemplaza por tap (pool) y long-press para drag. **N/A: Pool no es visible en mobile**.
...
1. **Revisión de código (arquitectura)**
...
2. **Revisión visual con Playwright (headless)**
   - Pool: validar mockup base + popup lateral en hover/tap (icono, titulo, color lateral, info secundaria).
   - Lista: validar fila con icono tipo, titulo, usuario + estado operativo (▶/⏸) sin card padre.
...
|-------|---------|-------------|-------|
| 2026-01-27 | 1.1 | Implementacion completa de vistas unificadas (helpers, estilos, bordes por card, tamaño uniforme en Pool), tests y QA visual | James (Dev) |
...
- Set pool canvas cards to uniform size (110px) to remove priority-driven size variance.
...
- Coding Standards: ✓ (helpers are view-only, naming and module placement align with conventions)
- Project Structure: ✓ (new UI helpers under `apps/client/src/scrumbringer_client/ui`)
- Testing Strategy: ⚠️ (unit/render tests present; Playwright evidence referenced but not re-run)
- All ACs Met: ✗ (AC9 missing: Pool tap/long-press behavior on mobile/tablet not implemented)
...
Gate: CONCERNS → docs/qa/gates/5.1-unificacion-ux-de-vistas-de-tareas.yml
...
AC9 implemented with tap-to-preview and long-press drag handling. Pool card size adjusted to 128px (8rem). Visual QA re-run with fresh screenshots.
...
- Coding Standards: ✓
- Project Structure: ✓
- Testing Strategy: ✓ (client tests + Playwright visual QA)
- All ACs Met: ✓

### Improvements Checklist

- [x] Implement Pool touch behavior (tap for preview, long-press for drag) (AC9)
- [x] Align Pool card size to spec minimum (8rem)
- [x] Re-run Playwright visual QA and attach evidence

### Security Review

No auth/data changes; no security concerns.

### Performance Considerations

No regressions observed; touch handlers add minimal overhead.
...
Gate: PASS → docs/qa/gates/5.1-unificacion-ux-de-vistas-de-tareas.yml

### Evidence

- `/tmp/visual-qa-pool-full.png`
- `/tmp/visual-qa-pool-tap.png`
- `/tmp/visual-qa-list-full.png`
- `/tmp/visual-qa-kanban-full.png`
- `/tmp/visual-qa-right-panel.png`
...
Reviewed Pool, Lista (grouped list), Kanban, Sidebar, and shared styles for the unified semantics. The DRY helpers are applied consistently and the sidebar sections align with the story’s hierarchy. The remaining functional gap is Pool touch behavior (tap/long-press), which is still mouse-only.
...
- Coding Standards: ✓ (modules and view-only helpers stay within expected boundaries)
- Project Structure: ✓ (UI helpers in `apps/client/src/scrumbringer_client/ui` and views under `features`)
- Testing Strategy: ⚠️ (tests not re-run for this review)
- All ACs Met: ✗ (AC9 missing: Pool tap/long-press behavior on mobile/tablet)

### Improvements Checklist

- [ ] Implement Pool touch behavior (tap to open popup, long-press to drag) to satisfy AC9.
- [ ] Align Pool card size with spec (min 8rem, ideal 10rem; current size 110px).
- [ ] Add a focused interaction test for Pool touch once behavior is implemented.

### Security Review

No new auth/data handling changes observed.

### Performance Considerations

UI-only changes; no obvious performance regressions in reviewed files.
...
Gate: FAIL → docs/qa/gates/5.1-unificacion-ux-de-vistas-de-tareas.yml
