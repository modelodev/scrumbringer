...
```
▌ [🔧]  Fix login button   Sprint Planning   Reclamada por admin ▶   ⋯
```
...
7. Sidebar no muestra progreso ni responsable ni estado.
...
1. **Icono de tipo**
   - Nuevo modulo: `ui/task_type_icon.gleam`
   - Firma sugerida:
     ```gleam
...
2. **Borde lateral por color de card**
   - Nuevo modulo: `ui/task_color.gleam`
   - Firma sugerida:
     ```gleam
...
3. **Popup lateral de Pool**
   - Nuevo modulo: `ui/task_hover_popup.gleam`
   - Firma sugerida:
     ```gleam
