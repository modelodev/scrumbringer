# Story 4.10: Asociar Plantillas a Reglas

## Status

Ready

## Estimación

- **Esfuerzo**: 8 story points (~3 días)
- **Prioridad**: Must Have (sin esto las reglas no crean tareas)

## Story

**As a** PM o Admin de Scrumbringer,
**I want** poder asociar plantillas a reglas desde la vista de reglas,
**so that** las reglas puedan crear tareas automáticamente cuando se activen.

## Prerequisites

- **Story 3.2** (Motor de reglas v1) - Backend completo
- **Story 4.9** (Completitud de CRUDs) - Vista plantillas funcional

## Problem Statement

El backend para asociar plantillas a reglas está implementado (Story 3.2), pero **no existe UI** para hacerlo. Esto significa que:

1. Las reglas no pueden tener plantillas asociadas desde la interfaz
2. Las reglas activas **nunca crean tareas** porque no tienen plantillas
3. El sistema de automatización está incompleto funcionalmente

### API Disponible

| Endpoint | Estado | Descripción |
|----------|--------|-------------|
| `POST /api/v1/rules/:id/templates/:tid` | ✅ Existe | Asociar plantilla a regla |
| `DELETE /api/v1/rules/:id/templates/:tid` | ✅ Existe | Desasociar plantilla |
| `GET /api/v1/workflows/:id/rules` | ⚠️ **Incompleto** | NO incluye `templates[]` - requiere Fase 0 |

### Infraestructura Cliente Existente

**YA IMPLEMENTADO** - Reutilizar, no duplicar:

| Componente | Ubicación | Estado |
|------------|-----------|--------|
| `rules_templates: Remote(List(RuleTemplate))` | `client_state.gleam:433` | ✅ Existe |
| `handle_rule_templates_fetched_ok/error` | `admin/workflows.gleam:364-379` | ✅ Existe |
| `api_workflows.attach_template()` | `api/workflows.gleam:362-386` | ✅ Existe |
| `api_workflows.detach_template()` | `api/workflows.gleam:388-403` | ✅ Existe |
| `RuleTemplate` type | `domain/workflow.gleam` | ✅ Existe |

**FALTA** - La conexión UI:

| Componente | Estado |
|------------|--------|
| Vista expandible en tabla de reglas | ❌ No existe |
| Modal para asociar plantillas | ❌ No existe |
| Indicador ⚠️ para reglas incompletas | ❌ No existe |

---

## Decisiones de Diseño

### 1. Vista Expandible en Tabla de Reglas

Las reglas se muestran en tabla. Al hacer clic en una fila, se expande para mostrar plantillas asociadas:

```
┌─────────────────────────────────────────────────────────────────┐
│ Feature → QA Review    Task→completed  ✓   ✏️ 🗑️              │
│ ▼ 2 plantillas asociadas                        [+ Asociar]    │
├─────────────────────────────────────────────────────────────────│
│   QA Review: {{father}}         🐛 Bug   P3              [✕]   │
│   Notify Team: {{user}}         📋 Task  P2              [✕]   │
└─────────────────────────────────────────────────────────────────┘
```

### 2. Indicador Visual de Regla Incompleta

Una regla sin plantillas muestra warning (⚠️) porque no creará tareas:

```
│ Bug Fix → Test         Task→completed  ⚠️  ✏️ 🗑️              │
│ ▶ Sin plantillas (no creará tareas)                            │
```

### 3. Modal Simple para Asociar

Solo plantillas del proyecto actual, sin las ya asociadas:

```
┌─────────────────────────────────────────────────────────────────┐
│ Asociar Plantilla                                        [✕]   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Plantillas disponibles en este proyecto:                       │
│                                                                 │
│  ○ QA Review: {{father}}              🐛 Bug    P3             │
│  ○ Regression Test: {{father}}        🧪 Test   P2             │
│  ● Deploy Checklist                   📋 Task   P4      ← sel  │
│                                                                 │
│  ℹ️ Las plantillas ya asociadas no aparecen en esta lista      │
│                                                                 │
│                              [Cancelar]  [Asociar]              │
└─────────────────────────────────────────────────────────────────┘
```

### 4. Estado Vacío Empático

Si una regla expandida no tiene plantillas:

```
│   💡 Asocia una plantilla para que esta regla cree tareas      │
│      automáticamente cuando se active.          [+ Asociar]    │
```

### 5. Sin Reordenación

El orden de plantillas es el de asociación. No se implementa drag-and-drop.

---

## Acceptance Criteria

### AC1-5: Vista Expandible de Reglas

- [ ] AC1: Al hacer clic en fila de regla, se expande/colapsa
- [ ] AC2: Toda la fila es clickeable (no solo el icono ▶/▼)
- [ ] AC3: Fila expandida muestra lista de plantillas asociadas
- [ ] AC4: Cada plantilla muestra: nombre, tipo (icono), prioridad, botón [✕]
- [ ] AC5: `aria-expanded` correcto en fila expandible

### AC6-8: Indicador de Regla Incompleta

- [ ] AC6: Regla sin plantillas muestra ⚠️ en lugar de ✓
- [ ] AC7: Texto "Sin plantillas (no creará tareas)" visible
- [ ] AC8: Regla con 1+ plantillas muestra ✓

### AC9-12: Modal Asociar Plantilla

- [ ] AC9: Botón [+ Asociar] abre modal
- [ ] AC10: Modal muestra solo plantillas del proyecto actual
- [ ] AC11: Plantillas ya asociadas NO aparecen en el modal
- [ ] AC12: Radio buttons para selección única

### AC13-15: Estados Vacíos

- [ ] AC13: Regla expandida sin plantillas muestra hint empático
- [ ] AC14: Modal sin plantillas disponibles muestra "No hay plantillas" + link
- [ ] AC15: Link a `/config/templates` funcional

### AC16-19: Operaciones CRUD

- [ ] AC16: Asociar plantilla (POST) funciona y actualiza lista
- [ ] AC17: Desasociar plantilla (DELETE con [✕]) funciona
- [ ] AC18: Toast "Plantilla asociada" tras éxito
- [ ] AC19: Toast "Plantilla desasociada" tras éxito

### AC20-22: Feedback y UX

- [ ] AC20: Loading state en botón [Asociar] durante submit
- [ ] AC21: Loading state en [✕] durante detach
- [ ] AC22: Error toast si operación falla

### AC23: Backend (Fase 0)

- [ ] AC23: `GET /api/v1/workflows/:id/rules` incluye `templates[]` para cada regla

---

## Tasks / Subtasks

### Fase 0: Modificación Tipo Rule (OBLIGATORIA - Bajo esfuerzo)

> ⚠️ **CRÍTICO**: El servidor actualmente NO devuelve `templates[]` en rules.
> Esta fase DEBE completarse antes de cualquier trabajo de UI.

- [ ] **0.1** Añadir `templates: List(RuleTemplate)` al tipo `Rule` en `shared/src/domain/workflow.gleam`
- [ ] **0.2** Actualizar decoder `rule_decoder()` en `api/workflows.gleam` para incluir `templates`
- [ ] **0.3** Actualizar servidor para incluir `templates[]` en respuesta de `/api/v1/workflows/:id/rules`
- [ ] **0.4** Verificar que tests existentes siguen pasando

### Fase 1: Estado y Mensajes (Bajo esfuerzo)

- [ ] **1.1** Añadir estado en `client_state.gleam`:
  - `rules_expanded: Set(Int)` - IDs de reglas expandidas
  - `attach_template_modal: Option(Int)` - rule_id si modal abierto
  - `attach_template_selected: Option(Int)` - template_id seleccionado
  - `attach_template_loading: Bool` - loading durante operación
  - `detaching_templates: Set(#(Int, Int))` - pares (rule_id, template_id) en proceso

- [ ] **1.2** Añadir mensajes en `client_state.gleam`:
  - `RuleExpandToggled(rule_id: Int)`
  - `AttachTemplateModalOpened(rule_id: Int)`
  - `AttachTemplateModalClosed`
  - `AttachTemplateSelected(template_id: Int)`
  - `AttachTemplateSubmitted`
  - `AttachTemplateSucceeded(rule_id: Int, templates: List(RuleTemplate))`
  - `AttachTemplateFailed(error: ApiError)`
  - `TemplateDetachClicked(rule_id: Int, template_id: Int)`
  - `TemplateDetachSucceeded(rule_id: Int, template_id: Int)`
  - `TemplateDetachFailed(error: ApiError)`

- [ ] **1.3** Añadir i18n keys en `text.gleam`, `es.gleam`, `en.gleam`:
  - `TemplatesAttached(n: Int)`
  - `NoTemplatesAttached`
  - `NoTemplatesWontCreateTasks`
  - `AvailableTemplatesInProject`
  - `AttachTemplateHint`
  - `NoTemplatesInProject`
  - `CreateTemplateLink`
  - `TemplateAttached`
  - `TemplateDetached`
  - `RemoveTemplate`

### Fase 2: Vista Expandible (Medio esfuerzo)

- [ ] **2.1** Refactorizar `view_rules_table` para filas expandibles
- [ ] **2.2** Implementar `view_rule_row_expandable` con:
  - Click handler en toda la fila
  - Icono ▶/▼ según estado
  - `aria-expanded` attribute
- [ ] **2.3** Implementar `view_rule_templates_panel` con:
  - Lista de plantillas asociadas
  - Botón [✕] para cada una
  - Estado vacío con hint
  - Botón [+ Asociar]
- [ ] **2.4** Implementar indicador ⚠️ para reglas sin plantillas

### Fase 3: Modal Asociar (Medio esfuerzo)

- [ ] **3.1** Implementar `view_attach_template_modal`:
  - Lista de plantillas disponibles (filtradas)
  - Radio buttons
  - Botones Cancelar/Asociar
- [ ] **3.2** Filtrar plantillas: solo proyecto actual, excluir ya asociadas
- [ ] **3.3** Estado vacío con link a Templates
- [ ] **3.4** Loading state en botón Asociar

### Fase 4: Handlers (Medio esfuerzo)

- [ ] **4.1** Handler `RuleExpandToggled` - toggle en Set
- [ ] **4.2** Handler modal (open/close/select)
- [ ] **4.3** Handler `AttachTemplateSubmitted`:
  - Validar selección
  - Llamar `api_workflows.attach_template`
  - Actualizar estado
- [ ] **4.4** Handler `TemplateDetachClicked`:
  - Llamar `api_workflows.detach_template`
  - Actualizar estado
- [ ] **4.5** Handlers de éxito/error con toasts

### Fase 5: CSS y Polish (Bajo esfuerzo)

- [ ] **5.1** CSS para fila expandible (`.rule-row-expandable`)
- [ ] **5.2** CSS para panel de plantillas (`.rule-templates-panel`)
- [ ] **5.3** CSS para estado vacío (`.rule-templates-empty-hint`)
- [ ] **5.4** CSS para plantilla en lista (`.attached-template-row`)
- [ ] **5.5** CSS para indicador incompleto (`.rule-incomplete-indicator`)

### Fase 6: Testing

- [ ] **6.1** Test unitario: filtrado de plantillas disponibles
- [ ] **6.2** Test E2E: flujo completo attach/detach
- [ ] **6.3** Verificar todos los ACs

---

## Task → AC Mapping

| Tarea | Cumple AC | Descripción |
|-------|-----------|-------------|
| 0.1-0.4 | **AC23** | Infraestructura: templates en Rule type y server |
| 1.1 | - | Estado para expandir/modal |
| 1.2 | - | Mensajes para operaciones |
| 1.3 | AC18, AC19 | i18n para toasts y UI |
| 2.1 | AC1 | Tabla con filas expandibles |
| 2.2 | AC1, AC2, AC5 | Fila clickeable con aria |
| 2.3 | AC3, AC4, AC13 | Panel de plantillas |
| 2.4 | AC6, AC7, AC8 | Indicador ⚠️ |
| 3.1 | AC9, AC12 | Modal con radio buttons |
| 3.2 | AC10, AC11 | Filtrado correcto |
| 3.3 | AC14, AC15 | Estado vacío con link |
| 3.4 | AC20 | Loading en botón |
| 4.1 | AC1 | Toggle expand |
| 4.2 | AC9 | Modal lifecycle |
| 4.3 | AC16, AC18 | Attach con toast |
| 4.4 | AC17, AC19, AC21 | Detach con toast |
| 4.5 | AC22 | Error handling |
| 5.1-5.5 | - | Estilos visuales |
| 6.1-6.3 | - | Verificación |

---

## Coding Standards Compliance

Esta historia **DEBE** seguir las normas de `docs/architecture/coding-standards.md`:

### Componentes UI Requeridos

| Componente | Uso en esta historia | Referencia |
|------------|---------------------|------------|
| `icons.nav_icon()` | Icono ▶/▼ expandir, icono ⚠️ warning, icono tipo plantilla | `ui/icons.gleam` |
| `action_buttons.delete_icon_button()` | Botón [✕] para desasociar plantilla | `ui/action_buttons.gleam` |
| `dialog.view()` | Modal asociar plantilla | `ui/dialog.gleam` |
| `toast` | Feedback "Plantilla asociada/desasociada" | `ui/toast.gleam` |

### Patrones Requeridos

```gleam
// ✅ CORRECTO: Usar icons.nav_icon para iconos
icons.nav_icon(icons.ChevronRight, icons.Small)  // ▶
icons.nav_icon(icons.Warning, icons.Small)        // ⚠️

// ✅ CORRECTO: Usar action_buttons para botones de acción
action_buttons.delete_icon_button(
  t(i18n_text.RemoveTemplate),
  TemplateDetachClicked(rule.id, template.id),
)

// ❌ INCORRECTO: No crear botones ad-hoc con iconos
button([class("btn-delete")], [text("✕")])  // No hacer esto
```

### TEA Pattern

- **View functions puras**: Sin efectos en view
- **Efectos en update**: Llamadas API solo desde handlers
- **Keyed lists**: Usar `lustre/element/keyed` para listas dinámicas

### Iconos a Usar (de `ui/icons.gleam`)

| Uso | Icono | Código |
|-----|-------|--------|
| Expandir/colapsar fila | ChevronRight/ChevronDown | `icons.nav_icon(icons.ChevronRight, icons.Small)` |
| Regla incompleta | Warning | `icons.nav_icon(icons.Warning, icons.Small)` |
| Regla completa | Check | `icons.nav_icon(icons.Check, icons.Small)` |
| Desasociar plantilla | Trash | Usar `action_buttons.delete_icon_button()` |
| Asociar plantilla | Plus | `icons.nav_icon(icons.Plus, icons.Small)` |

---

## Technical Notes

### Source Tree - Archivos a Modificar

```
shared/src/domain/
└── workflow.gleam                  ← MODIFICAR: añadir templates a Rule

apps/server/src/scrumbringer_server/
└── http/rules.gleam                ← MODIFICAR: incluir templates en response

apps/client/src/scrumbringer_client/
├── client_state.gleam              ← MODIFICAR: estado y mensajes
├── client_update.gleam             ← MODIFICAR: handlers
├── features/admin/
│   ├── view.gleam                  ← MODIFICAR: vista reglas expandible
│   └── workflows.gleam             ← MODIFICAR: handlers attach/detach
├── api/
│   └── workflows.gleam             ← MODIFICAR: decoder Rule con templates
├── i18n/
│   ├── text.gleam                  ← MODIFICAR: nuevos keys
│   ├── es.gleam                    ← MODIFICAR: traducciones ES
│   └── en.gleam                    ← MODIFICAR: traducciones EN
└── styles.gleam                    ← MODIFICAR: CSS expandible
```

### Estado en Model

```gleam
// En Model (client_state.gleam)
type Model {
  // ... existing fields ...

  // YA EXISTE (Story 3.2) - NO DUPLICAR:
  rules_templates: Remote(List(RuleTemplate)),  // Ya en línea 433

  // NUEVO - Story 4.10: Rule template attachment
  rules_expanded: Set(Int),                      // IDs de reglas expandidas
  attach_template_modal: Option(Int),            // rule_id si modal abierto
  attach_template_selected: Option(Int),         // template_id seleccionado
  attach_template_loading: Bool,                 // loading durante attach
  detaching_templates: Set(#(Int, Int)),         // (rule_id, template_id) siendo desasociados
}
```

**IMPORTANTE**: El campo `rules_templates` ya existe. Solo añadir los campos nuevos.

### Mensajes Nuevos

```gleam
// En Msg (client_state.gleam)
pub type Msg {
  // ... existing messages ...

  // Story 4.10: Rule template attachment
  RuleExpandToggled(Int)
  AttachTemplateModalOpened(Int)
  AttachTemplateModalClosed
  AttachTemplateSelected(Int)
  AttachTemplateSubmitted
  AttachTemplateSucceeded(Int, List(RuleTemplate))
  AttachTemplateFailed(ApiError)
  TemplateDetachClicked(Int, Int)  // rule_id, template_id
  TemplateDetachSucceeded(Int, Int)
  TemplateDetachFailed(Int, Int, ApiError)  // rule_id, template_id, error
}
```

**Nota**: `TemplateDetachFailed` incluye `rule_id` y `template_id` para poder quitar el loading state del par específico.

### i18n Keys

| Key | ES | EN |
|-----|----|----|
| `TemplatesAttached(n)` | `{n} plantillas asociadas` | `{n} templates attached` |
| `NoTemplatesAttached` | `Sin plantillas asociadas` | `No templates attached` |
| `NoTemplatesWontCreateTasks` | `Sin plantillas (no creará tareas)` | `No templates (won't create tasks)` |
| `AvailableTemplatesInProject` | `Plantillas disponibles en este proyecto` | `Available templates in this project` |
| `AttachTemplateHint` | `Asocia una plantilla para que esta regla cree tareas automáticamente cuando se active.` | `Attach a template so this rule creates tasks automatically when triggered.` |
| `NoTemplatesInProject` | `No hay plantillas en este proyecto` | `No templates in this project` |
| `CreateTemplateLink` | `Créala en Plantillas` | `Create one in Templates` |
| `TemplateAttached` | `Plantilla asociada` | `Template attached` |
| `TemplateDetached` | `Plantilla desasociada` | `Template detached` |
| `RemoveTemplate` | `Quitar plantilla` | `Remove template` |

### CSS Nuevo

```css
/* Fila expandible */
.rule-row-expandable {
  cursor: pointer;
}

.rule-row-expandable:hover {
  background: var(--sb-surface-hover);
}

.rule-row-expandable:focus-visible {
  outline: 2px solid var(--sb-primary);
  outline-offset: -2px;
}

/* Icono expand */
.rule-expand-icon {
  transition: transform 0.2s ease;
  margin-right: 8px;
}

.rule-row-expanded .rule-expand-icon {
  transform: rotate(90deg);
}

/* Indicador de regla incompleta */
.rule-incomplete-indicator {
  color: var(--sb-warning);
}

/* Panel de plantillas expandido */
.rule-templates-panel {
  background: var(--sb-surface-elevated);
  border-top: 1px solid var(--sb-border);
  padding: 12px 16px;
}

/* Estado vacío empático */
.rule-templates-empty-hint {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 8px;
  padding: 16px;
  color: var(--sb-muted);
  text-align: center;
}

.rule-templates-empty-hint .hint-icon {
  font-size: 1.5em;
}

/* Plantilla en lista */
.attached-template-row {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 8px 12px;
  border-radius: 6px;
  background: var(--sb-surface);
  margin-bottom: 6px;
}

.attached-template-row:hover {
  background: var(--sb-surface-hover);
}

.attached-template-info {
  display: flex;
  align-items: center;
  gap: 12px;
}

.attached-template-name {
  font-weight: 500;
}

.attached-template-meta {
  display: flex;
  align-items: center;
  gap: 8px;
  font-size: 0.85em;
  color: var(--sb-muted);
}

/* Header del panel */
.rule-templates-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 12px;
}
```

### Modificación Requerida al Tipo Rule

El tipo `Rule` en `shared/src/domain/workflow.gleam` **NO** incluye `templates`. Hay dos opciones:

**Opción A (RECOMENDADA)**: Añadir `templates` al tipo Rule

```gleam
// shared/src/domain/workflow.gleam
pub type Rule {
  Rule(
    id: Int,
    workflow_id: Int,
    name: String,
    goal: Option(String),
    resource_type: String,
    task_type_id: Option(Int),
    to_state: String,
    active: Bool,
    created_at: String,
    templates: List(RuleTemplate),  // ← AÑADIR
  )
}
```

**Ventajas**: Templates vienen con las reglas, no hay fetches adicionales.
**Requiere**: Modificar decoder en `api/workflows.gleam` y respuesta del servidor.

**Opción B**: Mantener fetch separado via `rules_templates`

**Ventajas**: No modifica tipos existentes.
**Desventajas**: Fetch adicional al expandir cada regla, UX más lenta.

**Decisión**: Usar **Opción A** para mejor UX.

### Lógica de Filtrado de Plantillas

```gleam
/// Filtra plantillas disponibles para asociar a una regla.
/// Excluye plantillas ya asociadas a esa regla.
fn available_templates_for_rule(
  all_templates: List(TaskTemplate),
  rule: Rule,
) -> List(TaskTemplate) {
  let attached_ids =
    rule.templates
    |> list.map(fn(rt) { rt.id })
    |> set.from_list

  all_templates
  |> list.filter(fn(t) { !set.contains(attached_ids, t.id) })
}
```

---

## Out of Scope

- Drag-and-drop para reordenar plantillas asociadas
- Bulk attach/detach (múltiples plantillas a la vez)
- Vista previa de tarea que se crearía con variables resueltas
- Búsqueda/filtro en modal de asociación

---

## Testing Strategy

### Unit Tests

| Test | Descripción |
|------|-------------|
| `available_templates_excludes_attached_test` | Filtrado correcto |
| `expand_toggle_updates_set_test` | Set de expandidos funciona |

### E2E Tests (Playwright)

| Test | ACs |
|------|-----|
| Click en fila expande/colapsa | AC1, AC2 |
| Regla sin plantillas muestra ⚠️ | AC6, AC7 |
| Modal muestra plantillas filtradas | AC10, AC11 |
| Attach actualiza lista | AC16, AC18 |
| Detach actualiza lista | AC17, AC19 |

---

## References

- **Story 3.2**: Motor de reglas v1 - Backend de templates attachment
- **Story 4.9**: Completitud de CRUDs - Vista plantillas
- **API**: `api/workflows.gleam` - `attach_template`, `detach_template`
- **Diseño UX**: Conversación con Sally (UX Expert) - 2026-01-23

---

## Change Log

| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 2026-01-23 | 0.1 | Story creada desde diseño UX | ux |
| 2026-01-23 | 0.2 | Refinada por Architect: coding standards, infraestructura existente, Fase 0, tipo Rule | architect |
| 2026-01-23 | 0.3 | Validada por PO: AC23 añadido, Fase 0 obligatoria, estimación y prioridad | po |
