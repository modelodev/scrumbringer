...
4. **Guardrail: cannot remove last org admin**: Attempting to demote the last remaining org admin is rejected.
   - Suggested: `409` error code `CONFLICT_LAST_ORG_ADMIN`.
5. **UI handles guardrail**: UI shows a clear inline error when the last-admin guardrail triggers, and the screen remains usable (no crash).
