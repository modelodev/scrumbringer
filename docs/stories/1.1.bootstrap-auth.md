...
2. **Invite-only registration (after bootstrap)**: If the organization already exists, `POST /api/v1/auth/register` with body `{ email, password, invite_token }`:
   - requires `invite_token` (missing invite is rejected with error code `INVITE_REQUIRED`),
   - rejects invalid/expired/used invite tokens with error codes `INVITE_INVALID`, `INVITE_EXPIRED`, `INVITE_USED` respectively,
