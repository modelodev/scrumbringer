...
Re-checked implementation against ACs: org-scoped capability listing is name-sorted; capability creation is org-admin only with org-scoped uniqueness; `/api/v1/me/capabilities` supports replace semantics (including clearing with `[]`) and rejects cross-org capability IDs without corrupting the existing selection.

### Evidence

- `apps/server` tests passed with a live Postgres test DB:
  - `DATABASE_URL=postgres://scrumbringer:scrumbringer@localhost:5433/scrumbringer_test?sslmode=disable ../../.tools/gleam-1.14.0/gleam test`

### Findings / Follow-ups

- Local verification via `make test` currently fails because `GLEAN_BIN` is repo-root-relative but executed after `cd apps/server`.
- `packages/birl` currently fails `gleam test` due to a Gleam module/native filename clash (`birl.gleam` vs `birl.erl`), which also impacts repo-level CI verification.

### Gate Status

Gate: CONCERNS → docs/qa/gates/1.4-capabilities-my-skills.yml

### Recommended Status

✓ Ready for Done (story functionality) / ⚠️ Follow-ups recommended for repo-level test automation
...
Follow-up items from the prior CONCERNS gate are resolved:

- `make test` now runs successfully from repo root (Makefile `GLEAN_BIN` uses an absolute path via `ROOT := $(abspath $(CURDIR))`).
- Removed generated Erlang artifacts in `packages/birl/src/` that caused the Gleam module/native name clash.
- `packages/birl` includes `test/birl_test.gleam`; `gleam test` now succeeds.

### Evidence

- `make test` (repo root): PASS
