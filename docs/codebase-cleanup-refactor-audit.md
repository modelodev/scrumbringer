...
| Lifecycle de tarea calculado en mas de un sitio | `domain/task/state.gleam` y presenter como conversion JSON | Derivar `status`/`work_state` desde `TaskExecutionState` | Test con datos redundantes inconsistentes que demuestre que gana el ADT | Test de presenter y barrido de `task_state.to_status`/`to_work_state` |
...
| `features/admin/rule_metrics.gleam` | contratos de route, auth, `try_update` e `init_tab` | rango de fechas, refresh, quick range, fetch ok/error, engine expansion, drilldown y paginacion de ejecuciones | `admin_rule_metrics_update_test.gleam` entra por `pool_messages.AdminRuleMetrics*`; `init_tab` sigue publico porque `client_update.gleam` inicializa la pestana |
