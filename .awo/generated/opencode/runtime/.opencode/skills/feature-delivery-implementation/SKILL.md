# Skill: feature-delivery-implementation

Use this skill only for workflow `feature_delivery` step `implementation`.
...
- `declared_skill_refs`: `[".opencode/skills/feature-delivery-implementation/SKILL.md"]`
- `materialized_skill_refs`: `[".opencode/skills/feature-delivery-implementation/SKILL.md"]` (exact runtime skill file AWO materialized for this step)
...
- Dependencies already satisfied: change_plan
- Required inputs: change_plan
- Required outputs: implementation
- Context mode: `full`
- Done criteria: Code or docs changes are applied only in the planned areas, Existing Gleam, Lustre, shared, SQL, and UI patterns are reused where possible, Relevant contracts, i18n, authz, migrations, Squirrel regeneration, or db impacts are updated or explicitly declared untouched, Scope stays aligned with the source story and does not silently expand

Artifact contract details:

- `implementation`: emit a durable contract artifact before completing the step
