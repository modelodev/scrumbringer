# Skill: feature-delivery-verification

Use this skill only for workflow `feature_delivery` step `verification`.
...
- `declared_skill_refs`: `[".opencode/skills/feature-delivery-verification/SKILL.md"]`
- `materialized_skill_refs`: `[".opencode/skills/feature-delivery-verification/SKILL.md"]` (exact runtime skill file AWO materialized for this step)
...
- Dependencies already satisfied: implementation
- Required inputs: implementation
- Required outputs: verification
- Context mode: `isolated`
- Done criteria: Build result recorded, Test result recorded or environment blocker explained clearly, If persistence or SQL changed, dbmate-backed checks and Squirrel impact are covered or the blocker is stated, Acceptance criteria coverage and residual regression risks are summarized honestly

Artifact contract details:

- `verification`: emit a durable contract artifact before completing the step

## Verification commands (mandatory)

After completing this step, execute these commands and report results:
- `make build` -> must exit 0
- `make test` -> must exit 0

If any verification command fails, set step status to "warning" and include the failure in interruption_events.
