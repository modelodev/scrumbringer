# Skill: user-story-authoring-story_draft

Use this skill only for workflow `user_story_authoring` step `story_draft`.
...
- `declared_skill_refs`: `[".opencode/skills/user-story-authoring-story_draft/SKILL.md"]`
- `materialized_skill_refs`: `[".opencode/skills/user-story-authoring-story_draft/SKILL.md"]` (exact runtime skill file AWO materialized for this step)
...
- Dependencies already satisfied: brief_alignment_check
- Required inputs: repo_context_scan, brief_alignment_check
- Required outputs: story_draft
- Context mode: `full`
- Done criteria: Markdown story draft created or updated under docs/stories/, Acceptance criteria are observable and testable, Tasks or subtasks map back to acceptance criteria, Expected code, docs, UI, and persistence touch points are named concretely

Artifact contract details:

- `story_draft`: emit a durable contract artifact before completing the step
