# Skill: user-story-authoring-ready_check

Use this skill only for workflow `user_story_authoring` step `ready_check`.
...
- `declared_skill_refs`: `[".opencode/skills/user-story-authoring-ready_check/SKILL.md"]`
- `materialized_skill_refs`: `[".opencode/skills/user-story-authoring-ready_check/SKILL.md"]` (exact runtime skill file AWO materialized for this step)
...
- Dependencies already satisfied: story_solution_fit
- Required inputs: story_solution_fit
- Required outputs: ready_check
- Context mode: `isolated`
- Done criteria: Story is small enough for one implementation slice, Blocking ambiguities are called out explicitly or declared absent, Validation plan is reproducible from the repo and names the commands that matter, Brief alignment and solution fit notes remain reflected in the final story, Final verdict is recorded as READY or BLOCKED

Artifact contract details:

- `ready_check`: emit a durable contract artifact before completing the step
