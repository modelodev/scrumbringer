...
pub fn manual_card_close_blocks_when_descendant_claimed_test() {
  ...
  let task =
    card_task(
      1,
      card_id.new(1),
      task_state.Claimed(user_id.new(9), now, task_state.Taken),
    )
...
pub fn manual_card_close_closes_available_descendant_tasks_test() {
  ...
  let expected_task_state =
    task_state.Closed(task_state.ClosedByAncestor, now, user_id.new(7))
...
pub fn closed_done_counts_as_completed_test() {
  let task = root_task(task_state.Closed(task_state.Done, now, user_id.new(7)))
...
pub fn closed_manually_does_not_count_as_completed_test() {
  let task =
    root_task(task_state.Closed(task_state.ManuallyClosed, now, user_id.new(7)))
...
pub fn closed_card_outcome_completed_when_all_leaves_done_test() {
  ...
  let assert option.Some(closure.Completed) =
...
pub fn closed_card_outcome_not_completed_when_any_leaf_closed_without_done_test() {
  ...
  let manual =
    card_task(
      2,
      card_id.new(1),
      task_state.Closed(task_state.ManuallyClosed, now, user_id.new(7)),
    )
...
pub fn manual_card_close_preserves_existing_closed_task_reasons_test() {
  ...
  let task =
    card_task(
      1,
      card_id.new(1),
      task_state.Closed(task_state.Done, now, user_id.new(9)),
    )
  let tree = activation.WorkTree(cards: [card], tasks: [task])
