...
-spec be_ok({ok, DOK} | {error, any()}) -> DOK.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, DOP}) -> DOP.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(DOS)) -> DOS.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
