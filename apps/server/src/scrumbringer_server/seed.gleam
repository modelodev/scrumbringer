...
pub fn main() {
  ...
  case run_seed() {
    Ok(stats) -> {
      ...
      io.println(
        "  pm@example.com       - Org Member, Project Manager on Alpha",
      )
      io.println("  member@example.com   - Org Member, Project Member on Alpha")
      io.println(
        "  beta@example.com     - Org Member, Project Manager on Beta only",
      )
      io.println("")
    }
    Error(msg) -> {
...
fn run_seed() -> Result(SeedStats, String) {
  ...
  use admin_id <- result.try(query_int(
    db,
  ...
  use member_id <- result.try(upsert_user(
    db,
    org_id,
    "member@example.com",
    "member",
  ...
  use beta_user_id <- result.try(upsert_user(
    db,
    org_id,
    "beta@example.com",
    "member",
  ...
  io.println(
    "[OK] Alpha members: admin (manager), pm (manager), member (member)",
  )

  use alpha_bug_type <- result.try(insert_task_type(
    db,
    alpha_id,
    "Bug",
    "bug-ant",
  ))
  use alpha_feature_type <- result.try(insert_task_type(
    db,
    alpha_id,
    "Feature",
    "sparkles",
  ))
  use alpha_task_type <- result.try(insert_task_type(
    db,
    alpha_id,
    "Task",
  ...
  use alpha_review_tmpl <- result.try(insert_template(
    db,
    org_id,
    alpha_id,
    alpha_task_type,
  ...
  use alpha_qa_tmpl <- result.try(insert_template(
    db,
    org_id,
    alpha_id,
    alpha_task_type,
  ...
  use alpha_deploy_tmpl <- result.try(insert_template(
    db,
    org_id,
    alpha_id,
    alpha_task_type,
  ...
  use wf_bug_id <- result.try(insert_workflow(
    db,
    org_id,
    alpha_id,
    "Bug Resolution",
  ...
  use wf_feature_id <- result.try(insert_workflow(
    db,
    org_id,
    alpha_id,
    "Feature Development",
  ...
  use wf_card_id <- result.try(insert_workflow(
    db,
    org_id,
    alpha_id,
    "Card Automation",
  ...
  use rule_bug_resolved <- result.try(insert_rule(
    db,
    wf_bug_id,
    "On Bug Resolved",
    "task",
  ...
  use rule_bug_closed <- result.try(insert_rule(
    db,
    wf_bug_id,
    "On Bug Closed",
    "task",
  ...
  use rule_feature_done <- result.try(insert_rule(
    db,
    wf_feature_id,
    "On Feature Done",
    "task",
  ...
  use rule_feature_qa <- result.try(insert_rule(
    db,
    wf_feature_id,
  ...
  use _rule_card <- result.try(insert_rule(
    db,
    wf_card_id,
    "On Card Archived",
    "card",
  ...
  use beta_bug_type <- result.try(insert_task_type(
    db,
    beta_id,
    "Bug",
    "bug-ant",
  ))
  use _beta_feature <- result.try(insert_task_type(
    db,
    beta_id,
    "Feature",
    "sparkles",
  ))
  use wf_beta_id <- result.try(insert_workflow(
    db,
    org_id,
    beta_id,
    "Simple Bug Flow",
  ...
  use _rule_beta <- result.try(insert_rule(
    db,
    wf_beta_id,
  ...
  use card_sprint <- result.try(insert_card(
    db,
    alpha_id,
    "Sprint Notes",
    Some("blue"),
  ...
  use card_arch <- result.try(insert_card(
    db,
    alpha_id,
    "Architecture",
    Some("purple"),
  ...
  use card_retro <- result.try(insert_card(
    db,
    alpha_id,
    "Retro",
    Some("green"),
  ...
  use card_release <- result.try(insert_card(
    db,
    alpha_id,
    "Release",
    Some("orange"),
  ...
  use bug1 <- result.try(insert_task(
    db,
    alpha_id,
    alpha_bug_type,
    "Login broken",
  ...
  use bug2 <- result.try(insert_task(
    db,
    alpha_id,
    alpha_bug_type,
    "Dashboard slow",
  ...
  use bug3 <- result.try(insert_task(
    db,
    alpha_id,
    alpha_bug_type,
    "Upload fails",
  ...
  use bug4 <- result.try(insert_task(
    db,
    alpha_id,
    alpha_bug_type,
    "Timeout",
  ...
  use bug5 <- result.try(insert_task(
    db,
    alpha_id,
    alpha_bug_type,
    "Email delayed",
  ...
  use feat1 <- result.try(insert_task(
    db,
    alpha_id,
    alpha_feature_type,
    "Dark mode",
  ...
  use feat2 <- result.try(insert_task(
    db,
    alpha_id,
    alpha_feature_type,
    "Export PDF",
  ...
  use feat3 <- result.try(insert_task(
    db,
    alpha_id,
    alpha_feature_type,
    "Notifications",
  ...
  use beta_card1 <- result.try(insert_card(
    db,
    beta_id,
    "Backend Refactor",
    Some("red"),
  ...
  use beta_card2 <- result.try(insert_card(
    db,
    beta_id,
    "API Cleanup",
    Some("yellow"),
  ...
  use beta_card3 <- result.try(insert_card(
    db,
    beta_id,
    "DB Migration",
    Some("pink"),
  ...
  use beta_card_empty <- result.try(insert_card(
    db,
    beta_id,
    "Docs",
    Some("gray"),
  ...
  use beta_bug1 <- result.try(insert_task(
    db,
    beta_id,
    beta_bug_type,
    "Beta Bug 1",
  ...
  use beta_bug2 <- result.try(insert_task(
    db,
    beta_id,
    beta_bug_type,
    "Beta Bug 2",
  ...
  use beta_bug3 <- result.try(insert_task(
    db,
    beta_id,
    beta_bug_type,
    "Beta Bug 3",
  ...
  use beta_bug4 <- result.try(insert_task(
    db,
    beta_id,
    beta_bug_type,
    "Beta Bug 4",
  ...
  use _ <- result.try(
    list.try_map(all_alpha_tasks, fn(id) {
      insert_task_event(
        db,
        org_id,
        alpha_id,
  ...
  use _ <- result.try(
    list.try_map(all_beta_tasks, fn(id) {
      insert_task_event(
        db,
        org_id,
        beta_id,
  ...
  let events_count =
    mut_events + list.length(all_alpha_tasks) + list.length(all_beta_tasks)
  io.println(
    "[OK] Created events: "
    <> int.to_string(list.length(all_alpha_tasks) + list.length(all_beta_tasks)),
  )
  ...
  use _ <- result.try(
    list.try_map(to_claim_alpha, fn(id) {
      use _ <- result.try(insert_task_event(
        db,
        org_id,
        alpha_id,
        id,
  ...
  use _ <- result.try(
    list.try_map(to_claim_beta, fn(id) {
      use _ <- result.try(insert_task_event(
        db,
        org_id,
        beta_id,
  ...
  let events_count =
    events_count + list.length(to_claim_alpha) + list.length(to_claim_beta)
  io.println(
    "[OK] Claimed events: "
    <> int.to_string(list.length(to_claim_alpha) + list.length(to_claim_beta)),
  )

  // Release some tasks (2)
  let to_release = list.take(to_claim_alpha, 2)
  use _ <- result.try(
    list.try_map(to_release, fn(id) {
      use _ <- result.try(insert_task_event(
        db,
        org_id,
        alpha_id,
  ...
  use _ <- result.try(
    list.try_map(to_complete_alpha, fn(id) {
      use _ <- result.try(insert_task_event(
        db,
        org_id,
        alpha_id,
  ...
  use _ <- result.try(
    list.try_map(to_complete_beta, fn(id) {
      use _ <- result.try(insert_task_event(
        db,
        org_id,
        beta_id,
  ...
  let events_count =
    events_count
    + list.length(to_complete_alpha)
    + list.length(to_complete_beta)
  io.println(
    "[OK] Completed events: "
    <> int.to_string(
      list.length(to_complete_alpha) + list.length(to_complete_beta),
    ),
  )
  ...
  // Beta bugs
  use _ <- result.try(
    list.try_map(beta_bug_ids, fn(id) {
      rules_engine.evaluate_rules(
        db,
        seed_task_event(
          id,
  ...
  io.println(
    "[OK] Beta bugs resolved: " <> int.to_string(list.length(beta_bug_ids)),
  )
...
fn upsert_user(
  ...
  let password_hash =
    "$argon2id$v=19$m=19456,t=2,p=1$Dqfb9+7qAiJzB5ghwAjP8A$3agIFIqxEfklBQ4Y+kbetHBD2hyyPZyEfqC8GPwkhDY"
  ...
  |> pog.execute(db)
  |> result.map_error(fn(e) {
    "Upsert user " <> email <> ": " <> string.inspect(e)
  })
  |> result.try(fn(r) {
    case r.rows {
      [id] -> Ok(id)
      _ -> Error("No ID for " <> email)
    }
  })
}
...
fn insert_template(
  ...
  pog.query(
    "INSERT INTO task_templates (org_id, project_id, type_id, name, description, priority, created_by) VALUES ($1, $2, $3, $4, 'Seeded', 3, $5) RETURNING id",
  )
  |> pog.parameter(pog.int(org_id))
  |> pog.parameter(pog.int(project_id))
...
fn insert_workflow(
  ...
  pog.query(
    "INSERT INTO workflows (org_id, project_id, name, active, created_by) VALUES ($1, $2, $3, true, $4) RETURNING id",
  )
  |> pog.parameter(pog.int(org_id))
  |> pog.parameter(pog.int(project_id))
  |> pog.parameter(pog.text(name))
...
fn insert_rule(
  ...
  pog.query(
    "INSERT INTO rules (workflow_id, name, resource_type, task_type_id, to_state, active) VALUES ($1, $2, $3, $4, $5, true) RETURNING id",
  )
...
fn insert_task(
  ...
  pog.query(
    "INSERT INTO tasks (project_id, type_id, title, description, priority, status, created_by, card_id) VALUES ($1, $2, $3, 'Seeded', 3, 'available', $4, $5) RETURNING id",
  )
  |> pog.parameter(pog.int(project_id))
  |> pog.parameter(pog.int(type_id))
...
fn insert_card(
  ...
  pog.query(
    "INSERT INTO cards (project_id, title, description, color, created_by) VALUES ($1, $2, 'Seeded', $3, $4) RETURNING id",
  )
  |> pog.parameter(pog.int(project_id))
  |> pog.parameter(pog.text(title))
...
fn insert_task_event(
  ...
  pog.query(
    "INSERT INTO task_events (org_id, project_id, task_id, actor_user_id, event_type) VALUES ($1, $2, $3, $4, $5)",
  )
  |> pog.parameter(pog.int(org_id))
  |> pog.parameter(pog.int(project_id))
  |> pog.parameter(pog.int(task_id))
  |> pog.parameter(pog.int(user_id))
  |> pog.parameter(pog.text(task_events_db.event_type_to_string(event_type)))
  |> pog.execute(db)
  |> result.map(fn(_) { Nil })
