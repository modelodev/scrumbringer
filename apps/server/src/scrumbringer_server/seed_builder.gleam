...
fn build_automation_diagnostics(
  ...
  case active_projects {
    [_default_project_id, _healthy_project_id, stress_project_id, ..] -> {
      use state <- result.try(seed_ignored_duplicate_execution(
        db,
        state,
        stress_project_id,
      ))
      seed_noisy_engine_executions(db, state, stress_project_id)
    }
    _ -> Ok(state)
  }
}
...
fn seed_noisy_engine_executions(
  ...
  case rules, templates, task_types, task_seeds {
    [rule_id, ..],
      [template_id, ..],
    ...
    -> {
      ...
      Ok(
        BuildState(
          ..state,
          task_ids: list.append(state.task_ids, created_task_ids),
