...
pub fn is_project_member_returns_true_for_member_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Member Test Project")
  ...
  let result = auth.is_project_member(db, user_id, project_id)
...
pub fn is_project_member_returns_false_for_non_member_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Non-Member Test Project")

  // Create a second user who is NOT added to the project
  let assert Ok(other_user_id) = fixtures.create_member_user(handler, db, "outsider@example.com", "inv_outsider")
  // Note: We intentionally do NOT call add_member

  // When: Check if non-member user is member of the project
  let result = auth.is_project_member(db, other_user_id, project_id)
...
pub fn is_project_admin_returns_true_for_admin_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Admin Test Project")
...
pub fn is_project_admin_returns_false_for_member_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Admin Negative Test Project")

  // Create a second user and add as regular member (not admin)
  let assert Ok(member_user_id) = fixtures.create_member_user(handler, db, "member@example.com", "inv_member")
...
pub fn org_admin_role_correctly_identified_test() {
  ...
  let assert Ok(org_role) = fixtures.query_string(
    db,
    "SELECT org_role FROM users WHERE email = $1",
...
pub fn org_member_role_correctly_identified_test() {
  ...
  let assert Ok(_) = fixtures.create_member_user(handler, db, "orgmember@example.com", "inv_orgmember")
