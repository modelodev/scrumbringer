...
import scrumbringer_server/services/rules_engine.{Applied, RuleResult, Suppressed}
...
pub fn evaluate_rule_applies_matching_rule_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Rule Apply Test")
  let assert Ok(type_id) = fixtures.create_task_type(handler, session, project_id, "Bug", "bug-ant")
  let assert Ok(workflow_id) = fixtures.create_workflow(handler, session, project_id, "Apply Test WF")
  let assert Ok(_rule_id) = fixtures.create_rule(handler, session, workflow_id, Some(type_id), "Bug Complete", "completed")
  let assert Ok(task_id) = fixtures.create_task(handler, session, project_id, type_id, "Test Bug")
...
pub fn evaluate_rule_skips_inactive_rule_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Inactive Rule Test")
  let assert Ok(type_id) = fixtures.create_task_type(handler, session, project_id, "Task", "check")
  let assert Ok(workflow_id) = fixtures.create_workflow(handler, session, project_id, "Inactive Rule WF")
...
pub fn evaluate_rule_handles_idempotent_suppression_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Idempotent Test")
  let assert Ok(type_id) = fixtures.create_task_type(handler, session, project_id, "Feature", "star")
  let assert Ok(workflow_id) = fixtures.create_workflow(handler, session, project_id, "Idempotent WF")
  let assert Ok(rule_id) = fixtures.create_rule(handler, session, workflow_id, Some(type_id), "Feature Done", "completed")
  let assert Ok(task_id) = fixtures.create_task(handler, session, project_id, type_id, "Test Feature")
  ...
  let assert Ok([RuleResult(rule_id: rid, outcome: Suppressed("idempotent"))]) = result2
...
pub fn evaluate_rule_skips_when_workflow_inactive_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Inactive WF Test")
  let assert Ok(type_id) = fixtures.create_task_type(handler, session, project_id, "Task", "check")
  let assert Ok(workflow_id) = fixtures.create_workflow(handler, session, project_id, "Inactive Workflow")
  let assert Ok(_rule_id) = fixtures.create_rule(handler, session, workflow_id, None, "Any Done", "completed")
