...
pub fn realistic_seed_includes_automation_traces_and_warnings_test() {
  // Covers the seed_automation_definitions, seed_automation_executions, and
  // seed_automation_diagnostics scenario modules.
  let assert Ok(#(db, _org_id, _admin_id)) = build_realistic_seed()
...
pub fn realistic_seed_covers_cards_tasks_and_due_dates_test() {
  // Covers seed_card_scenarios, seed_task_scenarios, seed_plan_scenarios, and
  // seed_root_card_scenarios coverage for cards, tasks, and due dates.
  let assert Ok(#(db, _org_id, _admin_id)) = build_realistic_seed()
