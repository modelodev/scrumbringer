...
pub fn complete_task_via_api_triggers_rules_and_creates_tasks_test() {
  ...
  let assert Ok(rule_id) =
    fixtures.create_rule(handler, session, workflow_id, Some(bug_type_id), "Bug Complete", "completed")
  let assert Ok(Nil) =
  ...
  let assert Ok(exec_count_before) =
    fixtures.query_int(db, "SELECT count(*)::int FROM rule_executions WHERE rule_id = $1", [pog.int(rule_id)])
  ...
  let claim_res = handler(
    simulate.request(http.Post, "/api/v1/tasks/" <> int.to_string(bug_task_id) <> "/claim")
  ...
  let complete_res = handler(
    simulate.request(http.Post, "/api/v1/tasks/" <> int.to_string(bug_task_id) <> "/complete")
    |> fixtures.with_auth(session)
...
pub fn complete_task_with_multiple_templates_creates_all_tasks_test() {
  ...
  let assert Ok(feature_type_id) =
    fixtures.create_task_type(handler, session, project_id, "Feature", "sparkles")
  let assert Ok(qa_type_id) =
    fixtures.create_task_type(handler, session, project_id, "QA", "magnifier")

  let assert Ok(workflow_id) =
    fixtures.create_workflow(handler, session, project_id, "Feature Complete WF")

  // Create 3 templates
  let assert Ok(template1_id) =
    fixtures.create_template(handler, session, project_id, qa_type_id, "QA Verification")
  let assert Ok(template2_id) =
    fixtures.create_template(handler, session, project_id, qa_type_id, "Deploy to Staging")
  let assert Ok(template3_id) =
    fixtures.create_template(handler, session, project_id, qa_type_id, "Code Review")

  // Create rule and attach all templates
  let assert Ok(rule_id) =
    fixtures.create_rule(handler, session, workflow_id, Some(feature_type_id), "Feature Done", "completed")
  ...
  let assert Ok(feature_task_id) =
    fixtures.create_task(handler, session, project_id, feature_type_id, "Implement Dark Mode")

  let assert Ok(qa_count_before) =
    fixtures.query_int(db, "SELECT count(*)::int FROM tasks WHERE type_id = $1", [pog.int(qa_type_id)])
  qa_count_before |> should.equal(0)

  // Claim and complete
  let _ = handler(
    simulate.request(http.Post, "/api/v1/tasks/" <> int.to_string(feature_task_id) <> "/claim")
    |> fixtures.with_auth(session)
    |> simulate.json_body(json.object([#("version", json.int(1))])),
  )

  let complete_res = handler(
    simulate.request(http.Post, "/api/v1/tasks/" <> int.to_string(feature_task_id) <> "/complete")
    |> fixtures.with_auth(session)
    |> simulate.json_body(json.object([#("version", json.int(2))])),
  )
...
pub fn completing_same_task_twice_is_idempotent_test() {
  ...
  let assert Ok(template_id) =
    fixtures.create_template(handler, session, project_id, review_type_id, "Review Task")
  let assert Ok(rule_id) =
    fixtures.create_rule(handler, session, workflow_id, Some(bug_type_id), "Bug Complete", "completed")
  let assert Ok(Nil) =
    fixtures.attach_template(handler, session, rule_id, template_id)
  ...
  let assert Ok(bug_task_id) =
    fixtures.create_task(handler, session, project_id, bug_type_id, "Bug to Fix")
  ...
  let complete_again_res = handler(
    simulate.request(http.Post, "/api/v1/tasks/" <> int.to_string(bug_task_id) <> "/complete")
    |> fixtures.with_auth(session)
...
pub fn inactive_rule_does_not_trigger_on_api_complete_test() {
  ...
  let assert Ok(template_id) =
    fixtures.create_template(handler, session, project_id, review_type_id, "Review")
  let assert Ok(rule_id) =
    fixtures.create_rule(handler, session, workflow_id, Some(type_id), "Task Done", "completed")
  let assert Ok(Nil) =
    fixtures.attach_template(handler, session, rule_id, template_id)
...
pub fn complete_task_with_card_creates_child_tasks_with_same_card_test() {
  ...
  let assert Ok(bug_task_id) =
    fixtures.create_task_with_card(handler, session, project_id, bug_type_id, card_id, "Fix Login Bug")

  // Verify bug task has the card
  let assert Ok(bug_card_id) =
    fixtures.query_nullable_int(
      db,
      "SELECT card_id FROM tasks WHERE id = $1",
      [pog.int(bug_task_id)],
    )
...
pub fn complete_task_without_card_creates_child_tasks_without_card_test() {
  ...
  let assert Ok(bug_task_id) =
    fixtures.create_task(handler, session, project_id, bug_type_id, "Fix Bug No Card")
