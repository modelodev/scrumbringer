...
pub fn full_lifecycle_create_claim_complete_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Lifecycle Test Project")
  let assert Ok(type_id) = fixtures.create_task_type(handler, session, project_id, "Feature", "sparkles")

  // Step 1: Create a task
  let assert Ok(task_id) = fixtures.create_task(handler, session, project_id, type_id, "Complete Me")

  // Verify task is in available status
  let assert Ok(status1) = fixtures.query_string(
    db,
  ...
  let assert Ok(status2) = fixtures.query_string(
    db,
    "SELECT status FROM tasks WHERE id = $1",
    [pog.int(task_id)],
  )
  ...
  let assert Ok(status3) = fixtures.query_string(
    db,
    "SELECT status FROM tasks WHERE id = $1",
    [pog.int(task_id)],
  )
  ...
  let assert Ok(claimed_by) = fixtures.query_int(
    db,
  ...
  let assert Ok(completed_at_check) = fixtures.query_int(
    db,
...
pub fn full_lifecycle_create_claim_release_claim_test() {
  ...
  let assert Ok(project_id) = fixtures.create_project(handler, session, "Release Test Project")
  ...
  let assert Ok(user2_id) = fixtures.create_member_user(handler, db, "user2@example.com", "inv_user2")
  let assert Ok(_) = fixtures.add_member(handler, session, project_id, user2_id, "member")
  let assert Ok(session2) = fixtures.login(handler, "user2@example.com", "passwordpassword")

  // Step 1: Create a task
  let assert Ok(task_id) = fixtures.create_task(handler, session, project_id, type_id, "Release Me")
  ...
  let assert Ok(claimed_by1) = fixtures.query_int(
    db,
    "SELECT claimed_by FROM tasks WHERE id = $1",
    [pog.int(task_id)],
  )
  ...
  let assert Ok(status_after_release) = fixtures.query_string(
    db,
    "SELECT status FROM tasks WHERE id = $1",
    [pog.int(task_id)],
  )
  ...
  let assert Ok(claimed_by2) = fixtures.query_int(
    db,
    "SELECT claimed_by FROM tasks WHERE id = $1",
    [pog.int(task_id)],
  )
  ...
  let assert Ok(status_final) = fixtures.query_string(
    db,
    "SELECT status FROM tasks WHERE id = $1",
    [pog.int(task_id)],
  )
