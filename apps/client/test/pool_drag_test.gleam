...
fn rect() -> member_pool.Rect {
  member_pool.Rect(left: 10, top: 20, width: 100, height: 50)
}

pub fn drag_to_claim_armed_moves_idle_to_pending_rect_test() {
  let next = drag.drag_to_claim_armed(default_pool(), True)

  let assert member_pool.PoolDragPendingRect = next.member_pool_drag
}
...
pub fn drag_to_claim_armed_preserves_rect_and_resets_over_state_test() {
  ...
  let assert member_pool.PoolDragDragging(
    over_my_tasks: False,
    rect: member_pool.Rect(left: 10, top: 20, width: 100, height: 50),
...
pub fn drag_to_claim_disarmed_clears_pool_drag_test() {
  ...
  let assert member_pool.PoolDragIdle = next.member_pool_drag
}
...
pub fn start_marks_task_pending_and_requests_drop_rect_test() {
  ...
  let assert member_pool.DragPending(7) = next.member_drag
  let assert member_pool.PoolDragPendingRect = next.member_pool_drag
}
...
pub fn offset_resolved_activates_matching_pending_drag_test() {
  ...
  let assert member_pool.DragActive(7, 3, 4) = next.member_drag
...
pub fn clear_resets_drag_state_test() {
  ...
  let assert member_pool.DragIdle = next.member_drag
  let assert member_pool.PoolDragIdle = next.member_pool_drag
}
