...
pub fn member_project_cards_ok_updates_store_and_pending_test() {
  ...
  |> should.equal([card_a])
  normalized_store.pending(next.member.member_cards_store) |> should.equal(0)
}
...
pub fn late_project_response_keeps_other_project_index_test() {
  ...
  normalized_store.get_by_project(next_a.member.member_cards_store, 20)
  |> should.equal([card_b])
}
...
pub fn pending_counts_across_multi_project_refresh_test() {
  ...
  normalized_store.pending(next_a.member.member_cards_store) |> should.equal(1)
