...
pub fn translate(text: Text) -> String {
  case text {
    ...
    text.PinNote -> "Fijar nota"
    text.UnpinNote -> "Desfijar nota"
    text.CannotPinNote -> "Solo el autor o un manager puede fijar esta nota"
    ...
    text.PinnedContext -> "Contexto fijado"
    text.OpenNotes -> "Abrir notas"
    text.MorePinnedNotes(count) -> "+" <> int.to_string(count) <> " en notas"
    ...
    text.PoolVisibilityAllOpen -> "Abiertas"
    ...
    text.CapabilityScopeLabel -> "Capacidades"
    text.ScopeAll -> "Todas"
    text.ScopeMine -> "Mías"
    ...
    text.PeoplePurpose ->
      "Estado operativo del equipo por trabajo propio, bloqueos y disponibilidad."
    text.PeopleAttentionLabel -> "Atención"
    ...
    text.PeopleBlockedCount(count) ->
      case count {
        1 -> "1 bloqueada"
        _ -> int.to_string(count) <> " bloqueadas"
      }
    ...
    text.PeopleFilterAttention -> "Atención"
    text.PeopleFilterFree -> "Disponibles"
    ...
    text.NoApiTokensYet -> "Aún no hay tokens API"
    text.FailedToLoadPrefix -> "Error cargando: "
    text.Project -> "Proyecto"
    text.Scopes -> "Permisos"
    text.PermissionRead -> "Leer"
    text.PermissionWrite -> "Escribir"
    text.ResourceProjects -> "Proyectos"
    ...
    text.Revoke -> "Revocar"
    text.Revoked -> "Revocado"
    text.Expired -> "Expirado"
    text.Active -> "Activo"
    ...
    text.AvailableCount -> "Disponibles"
    text.OngoingCount -> "En curso"
    text.WipCount -> "Trabajo en curso"
    text.HealthPanel -> "Salud del flujo"
    text.HealthFlow -> "Flujo"
    text.HealthRelease -> "Liberación"
    text.HealthTimeToFirstClaim -> "Tiempo hasta la primera reclamación"
    text.HealthOk -> "OK"
    ...
    text.ReleaseAllConfirmBody(count, user_name) ->
      ...
      <> " tareas de "
      <> user_name
      <> ". Las tareas volverán al pool."
    text.ReleaseAllSuccess(count, user_name) ->
      "Se liberaron " <> int.to_string(count) <> " tareas de " <> user_name
    text.ReleaseAllNone(user_name) -> user_name <> " no tiene tareas reclamadas"
    text.ReleaseAllError(user_name) ->
      "No se pudieron liberar tareas de " <> user_name
    text.ReleaseAllSelfError -> "No puedes liberar tus propias tareas"
    ...
    text.RuleBuilderNewRule -> "Nueva regla"
    text.RuleBuilderEditRule -> "Editar regla"
    ...
    text.TaskStateClosedHint -> "cerrada y sin acciones pendientes"
    text.TaskHeadlineAvailable -> "Lista para reclamar"
    text.TaskHeadlineClaimedByYou -> "En Mis tareas, lista para empezar"
    text.TaskHeadlineClaimedByOther -> "Reclamada por otra persona"
    text.TaskHeadlineClaimed -> "Reclamada"
    text.TaskHeadlineOngoingByYou -> "Estás trabajando ahora"
    text.TaskHeadlineOngoingByOther -> "En curso por otra persona"
    text.TaskHeadlineOngoing -> "En curso"
    text.TaskHeadlineClosed -> "Cerrada"
    ...
    text.TaskTemplateLastExecution -> "Última"
    text.TaskTemplateNeverExecuted -> "Nunca"
    ...
    text.RuleMetricsDrilldown -> "Detalles de ejecución"
    text.SuppressionBreakdown -> "Eventos ignorados"
    text.SuppressionIdempotent -> "Duplicado (ya procesado)"
    ...
    text.ProjectExecutionsSelectProject ->
      "Selecciona un proyecto para ver ejecuciones."
    text.ProjectExecutionsDiagnostics -> "Diagnostico por motor"
    text.ProjectExecutionsDateColumn -> "Fecha"
    text.ProjectExecutionsEngineColumn -> "Motor"
    text.ProjectExecutionsRuleColumn -> "Regla"
    ...
    text.OutcomeApplied -> "Creada"
    text.OutcomeSuppressed -> "Ignorada"
    ...
    text.CardProgressCount(closed, total) ->
      case total {
        ...
        _ -> {
          ...
          int.to_string(closed) <> " de " <> int.to_string(total) <> task_label
        }
      }
    ...
    text.CardSummaryNoWorkTitle -> "Definir trabajo"
    text.CardSummaryNoWorkBody ->
      "Esta tarjeta está activa, pero todavía no tiene tareas directas."
    text.CardSummaryBlockedTitle(count) ->
      int.to_string(count) <> " señal de bloqueo"
    text.CardSummaryBlockedBody ->
      "Resuelve los bloqueos antes de esperar un flujo sano en esta tarjeta."
    text.CardSummaryCompleteTitle -> "Trabajo completado"
    text.CardSummaryCompleteBody ->
      "Todas las tareas directas están cerradas. Revisa si la tarjeta puede cerrarse."
    text.CardSummaryFlowTitle -> "Trabajo en flujo"
    text.CardSummaryFlowBody ->
      "Las tareas directas están definidas y no hay bloqueos dominando esta tarjeta."
    text.CardSummaryNoDescription -> "Todavía no se ha escrito el propósito."
    ...
    text.TaskEditLocation -> "Ubicación"
    ...
    text.TaskAutomationOrigin -> "Origen"
    ...
    text.TaskAutomationRuleSignal(rule_id) ->
      "Creada por regla de automatización #" <> int.to_string(rule_id)
    text.TaskAutomationTemplateLabel(template_id) ->
      "Plantilla #" <> int.to_string(template_id)
    text.TaskAutomationTemplateFallback -> "Plantilla"
    text.TaskAutomationGoToAutomation -> "Ir a automatización"
