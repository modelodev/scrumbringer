...
pub type Labels {
  Labels(progress: String, closed: String, in_progress: String, pending: String)
}
...
pub fn view(config: Config(msg)) -> Element(msg) {
  ...
  div([attribute.class("progress-tooltip"), attribute.role("tooltip")], [
    ...
    div([attribute.class("progress-tooltip-breakdown")], [
      div([attribute.class("progress-tooltip-item closed")], [
        span([attribute.class("progress-icon")], [text("●")]),
        text(" " <> int.to_string(closed) <> " " <> labels.closed),
      ]),
