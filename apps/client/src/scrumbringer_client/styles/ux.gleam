...
pub fn css() -> List(String) {
  [
    ...
    // Card Show panel
    "html:has(.card-show), body:has(.card-show) { overflow: hidden; }",
    ".card-show { position: fixed; inset: 0; z-index: 1000; display: flex; align-items: stretch; justify-content: flex-end; padding: 12px; overflow: hidden; overscroll-behavior: contain; }",
    ".card-show .modal-backdrop { position: absolute; inset: 0; background: rgba(0,0,0,0.5); z-index: 1; }",
    ".modal-content.card-detail { border-color: color-mix(in oklab, var(--sb-card-accent, var(--sb-border)) 34%, var(--sb-border)); z-index: 2; position: relative; width: min(920px, calc(100vw - 48px)); max-width: 920px; height: 100%; min-height: 0; max-height: none; padding: 0; overflow: hidden; display: flex; flex-direction: column; }",
