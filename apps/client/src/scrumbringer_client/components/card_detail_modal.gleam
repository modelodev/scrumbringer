...
fn card_property_decoder() -> Decoder(Msg) {
  ...
  decode.success(CardReceived(card.Card(
    id: id,
...
fn task_decoder() -> Decoder(Task) {
  ...
  use ongoing_by <- decode.optional_field("ongoing_by", option.None, decode.optional(ongoing_by_decoder()))
  use title <- decode.field("title", decode.string)
  ...
  use created_by <- decode.field("created_by", decode.int)
  use claimed_by <- decode.optional_field("claimed_by", option.None, decode.optional(decode.int))
  use claimed_at <- decode.optional_field("claimed_at", option.None, decode.optional(decode.string))
  use completed_at <- decode.optional_field("completed_at", option.None, decode.optional(decode.string))
  use created_at <- decode.field("created_at", decode.string)
  use version <- decode.field("version", decode.int)
  use card_id <- decode.optional_field("card_id", option.None, decode.optional(decode.int))
  use card_title <- decode.optional_field("card_title", option.None, decode.optional(decode.string))
  use card_color <- decode.optional_field("card_color", option.None, decode.optional(decode.string))
...
fn update(model: Model, msg: Msg) -> #(Model, Effect(Msg)) {
  case msg {
    CardIdReceived(id) ->
      #(Model(..model, card_id: option.Some(id)), effect.none())

    CardReceived(card) ->
      #(Model(..model, card: option.Some(card)), effect.none())
    ...
    TasksReceived(tasks) ->
      #(Model(..model, tasks: Loaded(tasks)), effect.none())

    ToggleAddTaskForm ->
      #(Model(..model, add_task_open: !model.add_task_open), effect.none())

    TitleInput(title) ->
      #(Model(..model, add_task_title: title), effect.none())

    PrioritySelect(priority) ->
      #(Model(..model, add_task_priority: priority), effect.none())
...
fn validate_and_submit(model: Model) -> #(Model, Effect(Msg)) {
  ...
  case title == "" {
    True ->
      #(
        Model(..model, add_task_error: option.Some(t(model.locale, i18n_text.TitleRequired))),
        effect.none(),
      )
...
fn validate_and_submit_with_type(model: Model, title: String) -> #(Model, Effect(Msg)) {
  ...
  case project_id_opt {
    option.None ->
      #(
        Model(..model, add_task_error: option.Some(t(model.locale, i18n_text.SelectProjectFirst))),
        effect.none(),
      )
...
fn validate_task_type_and_submit(
  ...
  case list.first(model.task_types) {
    Error(_) ->
      #(
        Model(..model, add_task_error: option.Some(t(model.locale, i18n_text.TypeRequired))),
        effect.none(),
      )
    Ok(first_type) -> {
...
fn emit_task_created() -> Effect(Msg) {
  effect.from(fn(_dispatch) {
    emit_custom_event("task-created", json.null())
  })
}
        ...
        [
          ...
          input([
            ...
            attribute.placeholder(
              t(model.locale, i18n_text.TaskTitlePlaceholder),
            ),
        ...
        [
          ...
          button(
            [
              ...
              attribute.disabled(model.add_task_title == "" || model.add_task_in_flight),
            ],
