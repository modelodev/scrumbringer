...
pub fn policy_for(
  ...
  let is_closed = card.state == Cerrada
...
pub fn direct_child_cards(card: Card, cards: List(Card)) -> List(Card) {
  list.filter(cards, fn(candidate) { candidate.milestone_id == Some(card.id) })
}
...
pub fn move_destinations(card: Card, cards: List(Card)) -> List(Card) {
  ...
  |> list.filter(fn(candidate) {
    ...
    && candidate.id != option_to_disallowed_parent_id(card.milestone_id)
    && candidate.project_id == card.project_id
    && candidate.state != Cerrada
