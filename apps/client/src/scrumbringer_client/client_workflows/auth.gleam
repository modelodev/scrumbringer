...
/// Handle login error.
pub fn handle_login_finished_error(model: Model, err: api.ApiError) -> #(Model, Effect(Msg)) {
    ...
    False -> {
      ...
      case email == "" {
        ...
        False -> {
          ...
          #(model, api.request_password_reset(email, ForgotPasswordFinished))
        }
      }
    }
  }
...
pub fn handle_logout_clicked(model: Model) -> #(Model, Effect(Msg)) {
  #(Model(..model, toast: opt.None), api.logout(LogoutFinished))
}
