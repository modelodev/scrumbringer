# Skill: feature-delivery-change_plan

Use this skill only for workflow `feature_delivery` step `change_plan`.
...
- Required outputs: change_plan
- Context mode: `isolated`
- Done criteria: Source story or brief path identified, normally under docs/stories/ or docs/brief.md, Target client, server, shared, db, and docs files are listed concretely when applicable, Smallest useful validation command set is chosen from make build, make test, and persistence-related commands when needed, Blocking ambiguities are called out with explicit fallback to user_story_authoring when definition is not good enough

Artifact contract details:

- `change_plan`: emit a durable contract artifact before completing the step
